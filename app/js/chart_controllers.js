    ...
    .controller('ChartCtrl', [
      ...
      'ChartData', function ($rootScope, $scope, localStorageService, ChartInitFctry, $state, ChartDraw, ChartData) {
        ...
        $scope.updateChart = function () {
          ...
          $scope.numbers_high = [];
          $scope.titles_high = [];
          $scope.getBarHighValue = function () {
            angular.forEach(ChartData.summary.high, function (value, key) {
              $scope.numbers_high.push(value);
              $scope.titles_high.push(key);
            });
          };
          $scope.numbers_low = [];
          $scope.titles_low = [];
          $scope.getBarLowValue = function () {
            angular.forEach(ChartData.summary.low, function (value, key) {
              $scope.numbers_low.push(value);
              $scope.titles_low.push(key);
            });
          };
          var seriesArrayBar = $scope.chartConfig3.series[0];
          seriesArrayBar.data = $scope.numbers_high;

          $scope.getBarHighValue();
          $scope.getBarLowValue();
          console.log($scope.numbers_high);
          console.log($scope.titles_high);
          ...
          $scope.getPieHighValue = function () {
            var i = 0;
            angular.forEach(ChartData.summary.high, function (value, key) {
              $scope.pie_values_high.push([$scope.pie_titles[i], value]);
              i ++ ;
            });
          };
          $scope.pie_values_low = [];
          $scope.getPieLowValue = function () {
            angular.forEach(ChartData.summary.low, function (value, key) {
              $scope.pie_values_low.push([key, value]);
            });
          };
          var seriesArrayPie = $scope.chartConfig3.series[0];
          seriesArrayPie.data = $scope.pie_values_high;

          $scope.getPieHighValue();
          $scope.getPieLowValue();
          console.log($scope.pie_values_low);

        };

        // Highcharts config for 'high' chart
        ChartDraw.setValue('high');
        $scope.chartConfig = ChartDraw.chartConfig;

        // Highcharts config for 'low' chart
        ChartDraw.setValue('low');
        $scope.chartConfig2 = ChartDraw.chartConfig;

        // Highcharts config for bar chart
        //$scope.chartConfig3 = ChartDraw.chartConfigBar;

        // Highcharts config for pie chart
        $scope.chartConfig3 = ChartDraw.chartConfigPie;
