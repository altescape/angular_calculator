...
angular.module("myApp.directives", [ "LocalStorageModule" ]).directive("mainNav", function() {
    ...
    var fb_base_url = "https://luminous-fire-1327.firebaseio.com", ref = new Firebase(fb_base_url), authObj = $firebaseAuth(ref), authData = ref.getAuth(), num_uid = function() {
        ...
        return !1;
    }, broadcast_auth_event = function(msg) {
        $rootScope.$broadcast("authEvent", msg), $rootScope.$broadcast("isLoggedInMessage", msg.success);
...
}), angular.module("myApp.controllers", []).controller("InfoCtrl", [ "$rootScope", "$scope", "localStorageService", "infoData", "$interval", "addThing", function($rootScope, $scope, localStorageService, infoData, $interval) {
...
} ]).controller("AuthCtrl", [ "$rootScope", "$scope", "Authorise", "$state", "$firebase", function($rootScope, $scope, Authorise, $state) {
    $scope.loginUser = function() {
        $scope.show_loader = !0, Authorise.login($scope.auth.email, $scope.auth.password);
    }, $scope.$on("authEvent", function(event, broadcast) {
        if (broadcast.success === !0) $scope.show_loader = !1, $scope.logged_in = !0, $state.go("info"); else $scope.show_loader = !1, 
