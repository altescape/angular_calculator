...
angular.module("myApp.directives", [ "LocalStorageModule" ]).directive("mainNav", function() {
    ...
    var FB_BASE_URL = "https://luminous-fire-1327.firebaseio.com", ref = new Firebase(FB_BASE_URL), authObj = $firebaseAuth(ref), authData = ref.getAuth(), num_uid = function() {
        if (ref.getAuth()) {
    ...
    var get_items_from_fb = function() {
        var url = Authorise.fb_base_url + "/user_data/" + Authorise.num_uid() + "/", ref = new Firebase(url), sync = $firebase(ref);
        return sync.$asArray();
    }, fb_items_promise = function(items) {
        return items.$loaded().then(function() {
