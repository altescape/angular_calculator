...
angular.module("myApp.directives", [ "LocalStorageModule" ]).directive("mainNav", function() {
    ...
    {
        var codeVersion = 0, appIsCurrent = !0;
        !function() {
            Authorisation.loginObj.$getCurrentUser().then(function(user) {
                if (null !== user) {
                    var url = Authorisation.url + "app/version", ref = new Firebase(url), obj = $firebase(ref).$asObject();
    ...
    return $firebase(ref);
} ]).factory("Authorisation", [ "$firebase", "$firebaseSimpleLogin", function($firebase, $firebaseSimpleLogin) {
    var firebaseAddress = "https://luminous-fire-1327.firebaseio.com/", dataRef = new Firebase(firebaseAddress);
    return {
...
}), angular.module("myApp.controllers", []).controller("InfoCtrl", [ "$rootScope", "$scope", "localStorageService", "$firebaseSimpleLogin", "Authorisation", "infoData", "$interval", "appVersion", function($rootScope, $scope, localStorageService, $firebaseSimpleLogin, Authorisation, infoData, $interval) {
    Authorisation.loginObj.$getCurrentUser().then(function(user) {
        if ($scope.user_forms_ready = !0, null === user) infoData.loggedIn = !1; else $scope.user = user, 
        infoData.loggedIn = !0;
...
} ]).controller("ListSessionCtrl", [ "$scope", "$state", "$firebase", "$firebaseSimpleLogin", "Authorisation", "$timeout", "localStorageService", "infoData", "inputData", "allData", "$location", "$modal", function($scope, $state, $firebase, $firebaseSimpleLogin, Authorisation, $timeout, localStorageService, infoData, inputData, allData, $location, $modal) {
    $scope.createNewSession = function() {
        $state.go("new-calculation"), $scope.$emit("createNewSession", !0);
    }, Authorisation.loginObj.$getCurrentUser().then(function(user) {
        if (null === user) ; else {
            var userId = user.id, sync = $firebase(new Firebase(Authorisation.url + "user_data/" + userId + "/")), items = sync.$asArray();
            ...
            $scope.ele_load_status = "hide_on_loading", $scope.sessionCount = function() {
                ...
                $scope.deleteItemId = id, $timeout(function() {
                    items.$remove(itemToRemove).then(function() {
                        $state.go("saved-calculations");
                    });
                }, 400);
            }, $scope.copySession = function(id) {
                var sync = $firebase(new Firebase(Authorisation.url + "/user_data/" + userId + "/" + id)), item = sync.$asObject();
                item.$loaded().then(function() {
                    var copy_of_data = item.data, copy_of_info = item.info, copy_of_input = item.input;
                    copy_of_info.date = {
                        timestamp: Math.round(new Date().getTime() / 1e3),
                        date: new Date().toISOString()
                    }, $scope.items.$add({
...
} ]).controller("ModalConfirmUseCtrl", [ "$scope", "$window", "$firebase", "$firebaseSimpleLogin", "Authorisation", "$state", "allData", "infoData", "inputData", "localStorageService", "$modalInstance", "id", function($scope, $window, $firebase, $firebaseSimpleLogin, Authorisation, $state, allData, infoData, inputData, localStorageService, $modalInstance, id) {
    $scope.id = id, $scope.ok = function() {
        Authorisation.loginObj.$getCurrentUser().then(function(user) {
            if (null === user) ; else {
                var userId = user.id, sync = $firebase(new Firebase(Authorisation.url + "/user_data/" + userId + "/" + id)), item = sync.$asObject();
                item.$loaded().then(function() {
...
} ]).controller("SessionsDetailCtrl", [ "$rootScope", "$scope", "$routeParams", "$firebase", "$firebaseSimpleLogin", "Authorisation", "$state", "$stateParams", "chartData", function($rootScope, $scope, $routeParams, $firebase, $firebaseSimpleLogin, Authorisation, $state, $stateParams, chartData) {
    Authorisation.loginObj.$getCurrentUser().then(function(user) {
        if (null === user) ; else {
            var userId = user.id, sync = $firebase(new Firebase(Authorisation.url + "/user_data/" + userId + "/" + $stateParams.id)), item = sync.$asObject();
            item.$loaded().then(function() {
...
} ]).controller("SaveSessionCtrl", [ "$scope", "$state", "localStorageService", "$firebase", "$firebaseSimpleLogin", "$timeout", "infoData", "inputData", "allData", "Authorisation", function($scope, $state, localStorageService, $firebase, $firebaseSimpleLogin, $timeout, infoData, inputData, allData, Authorisation) {
    $scope.info.running_session = null !== localStorageService.get("current_key") ? !0 : !1, 
    $scope.saving = !1, $scope.saved = !1;
    {
        ...
        !function() {
            Authorisation.loginObj.$getCurrentUser().then(function(user) {
                if (null === user) $state.go("auth"); else isLoggedIn = !0;
            });
        }();
    }
    $scope.saveNewSession = function() {
        Authorisation.loginObj.$getCurrentUser().then(function(user) {
            if (null === user) $state.go("auth"); else {
                var userId = user.id, sync = $firebase(new Firebase(Authorisation.url + "/user_data/" + userId + "/")), items = sync.$asArray();
                $scope.saving = !0, items.$add({
                    info: localStorageService.get("info")
                }).then(function(ref) {
                    localStorageService.set("current_key", ref.name()), $scope.info.running_session = !0, 
        ...
        Authorisation.loginObj.$getCurrentUser().then(function(user) {
            if (null === user) $state.go("auth"); else if ($scope.info.running_session === !0) {
                $scope.saving = !0;
                var userId = user.id, url = Authorisation.url + "/user_data/" + userId + "/" + localStorageService.get("current_key"), ref = new Firebase(url), obj = $firebase(ref).$asObject();
    ...
    var syncToFirebase = function() {
        Authorisation.loginObj.$getCurrentUser().then(function(user) {
            if (null === user) $state.go("auth"); else if ($scope.info.running_session === !0) {
                $scope.saving = !0;
                var userId = user.id, url = Authorisation.url + "/user_data/" + userId + "/" + localStorageService.get("current_key"), ref = new Firebase(url), obj = $firebase(ref);
                obj.$set({
                    info: localStorageService.get("info"),
...
} ]).controller("AuthCtrl", [ "$rootScope", "$scope", "$state", "$firebase", "$firebaseSimpleLogin", "Authorisation", function($rootScope, $scope, $state, $firebase, $firebaseSimpleLogin, Authorisation) {
    Authorisation.loginObj.$getCurrentUser().then(function(user) {
        if ($scope.user_forms_ready = !0, null === user) $scope.$emit("isLoggedInMessage", !1), 
        $scope.logged_in = !1; else $scope.$emit("isLoggedInMessage", !0), $scope.user = user, 
        $scope.logged_in = !0;
    }), $scope.loginUser = function() {
        if ($scope.login.$valid) $scope.show_loader = !0, Authorisation.loginObj.$login("password", {
            email: $scope.auth.email,
            password: $scope.auth.password
        }).then(function() {
            $scope.logged_in = !0, $scope.$emit("isLoggedInMessage", !0), $state.go("info");
        }, function(error) {
            switch ($scope.show_loader = !1, $scope.message = "Login failed", error.code) {
              case "INVALID_EMAIL":
        ...
        $scope.logged_in = !1, $scope.$emit("isLoggedInMessage", !1), $scope.message = "", 
        Authorisation.loginObj.$logout();
    }, $scope.sendPassword = function() {
        if ($scope.show_loader = !0, $scope.password.$valid) Authorisation.loginObj.$sendPasswordResetEmail($scope.auth.email).then(function() {
            $scope.message = !0, $scope.show_loader = !1;
        });
    }, $scope.resetPassword = function() {
        if ($scope.show_loader = !0, $scope.reset.$valid) Authorisation.loginObj.$changePassword($scope.auth.email, $scope.auth.old_password, $scope.auth.new_password).then(function() {
            $scope.message = !0, $scope.show_loader = !1;
        });
    };
} ]);