...
angular.module("myApp.directives", [ "LocalStorageModule" ]).directive("mainNav", function() {
    ...
    var FB_BASE_URL = "https://luminous-fire-1327.firebaseio.com", ref = new Firebase(FB_BASE_URL), authObj = $firebaseAuth(ref), authData = authObj.$getAuth(), num_uid = function() {
        var user = authObj.$getAuth();
        if (user) {
            var uid = user.uid, result = uid.split(":");
            return result[1];
        }
        return !1;
    }, check_login_status = function() {
        var logged_in_user = authObj.$getAuth();
        if (logged_in_user) $rootScope.$broadcast("isLoggedInMessage", !0), $rootScope.$broadcast("isAuthorised", {
            status: !0,
            msg: logged_in_user.password.email
        }); else $rootScope.$broadcast("isAuthorised", {
            status: !1,
            msg: "Logged out"
        }), $state.go("auth");
        ...
        authObj.$authWithPassword({
            ...
            $rootScope.$broadcast("authEvent", {
                status: !0,
                msg: result
            }), $rootScope.$broadcast("isLoggedInMessage", !0), $state.go("info");
        })["catch"](function(error) {
            $rootScope.$broadcast("authEvent", {
                status: !1,
                msg: error.toString()
            }), $rootScope.$broadcast("isLoggedInMessage", !1);
        });
    }, logout = function() {
        authObj.$unauth(), $rootScope.$broadcast("isLoggedInMessage", !1), $rootScope.$broadcast("clearInfoData", !0);
    }, retrieve_password = function(email) {
    ...
    }, change_password = function(email, old_password, new_password) {
...
} ]).controller("ListSessionCtrl", [ "$scope", "$state", "localStorageService", "Authorise", "$modal", "$timeout", "infoData", "inputData", "allData", "$location", "$firebase", function($scope, $state, localStorageService, Authorise, $modal, $timeout, infoData, inputData, allData, $location, $firebase) {
    $scope.createNewSession = function() {
        $state.go("new-calculation"), $scope.$emit("createNewSession", !0);
    }, $scope.$on("isAuthorised", function(event, broadcast) {
        if (broadcast.status) fb_items_promise(get_items_from_fb()), $scope.current_key = localStorageService.get("current_key"), 
        $scope.load_status = "loading", $scope.ele_load_status = "hide_on_loading"; else $state.go("auth");
    ...
    var get_items_from_fb = function() {
        ...
        return console.log(sync), sync.$asArray();
    }, fb_items_promise = function(items) {
        return items.$loaded().then(function(data) {
            $scope.items = data, $scope.load_status = "loaded", $scope.ele_load_status = "show_on_loaded", 
            $scope.session_count = data.length;
        });
    }, copy_data = function(items) {
        var copy_of_data = items.data, copy_of_info = items.info, copy_of_input = items.input;
        return copy_of_info.date = {
            timestamp: Math.round(new Date().getTime() / 1e3),
            date: new Date().toISOString()
        }, {
            data: copy_of_data,
    ...
    }, save_a_copy = function(id) {
        var user_url = Authorise.fb_base_url + "/user_data/" + Authorise.num_uid(), user_ref = new Firebase(user_url), user_calculations = $firebase(user_ref).$asArray(), url = Authorise.fb_base_url + "/user_data/" + Authorise.num_uid() + "/" + id, ref = new Firebase(url), items = $firebase(ref).$asObject();
        items.$loaded().then(function() {
            user_calculations.$add(copy_data(items));
        });
    };
    $scope.deleteSession = function(id) {
        var url = Authorise.fb_base_url + "/user_data/" + Authorise.num_uid() + "/" + id, ref = new Firebase(url), obj = $firebase(ref).$asObject();
        obj.$remove().then(function() {
            return !0;
        }, function(error) {
            return console.log("Error:", error), !1;
        });
...
} ]).controller("ModalConfirmUseCtrl", [ "$scope", "$window", "localStorageService", "Authorise", "$state", "allData", "infoData", "inputData", "$modalInstance", "id", "$firebase", function($scope, $window, localStorageService, Authorise, $state, allData, infoData, inputData, $modalInstance, id, $firebase) {
    $scope.id = id, $scope.$on("isAuthorised", function(event, broadcast) {
        broadcast.status;
        if (broadcast.status) {
            if (get_item_from_fb()) {
    ...
    var get_item_from_fb = function() {
        var url = Authorise.fb_base_url + "/user_data/" + Authorise.num_uid() + "/" + id, ref = new Firebase(url), sync = $firebase(ref);
        return sync.$asObject();
    }, fb_item_promise = function(item) {
        return item.$loaded().then(function() {
...
} ]).controller("SessionsDetailCtrl", [ "$rootScope", "$scope", "$routeParams", "$firebase", "$firebaseAuth", "$state", "$stateParams", "chartData", function($rootScope, $scope, $routeParams, $firebase, $firebaseAuth, $state, $stateParams, chartData) {
...
} ]).controller("CalcCtrl", [ "$rootScope", "$scope", "Authorise", "$location", "localStorageService", "$state", "chartData", "chartConfig", "infoData", "inputData", "allData", "revenueIntegrity", "revenueIntegrityProcessImprovement", "cmap", "originAndDestination", "pointOfSale", "passengersBoardedData", "arr", "airfareInsight", "channelShift", "ancillarySales", "$firebase", "$interval", function($rootScope, $scope, Authorise, $location, localStorageService, $state, chartData, chartConfig, infoData, inputData, allData, revenueIntegrity, revenueIntegrityProcessImprovement, cmap, originAndDestination, pointOfSale, passengersBoardedData, arr, airfareInsight, channelShift, ancillarySales, $firebase) {
    $scope.$on("isAuthorised", function(event, broadcast) {
        var auth_status = broadcast.status;
        if (auth_status) if ($scope.info.running_session === !0) {
            $scope.saving = !0;
            var fb_base_url = Authorise.fb_base_url, url = fb_base_url + "/user_data/" + Authorise.num_uid() + "/" + localStorageService.get("current_key"), ref = new Firebase(url), obj = $firebase(ref);
            obj.$set({
                info: localStorageService.get("info"),
        ...
        localStorageService.set("input", inputData), $scope.input = inputData, $scope.syncToFirebase(), 
    ...
    }, $scope.pb_data = passengersBoardedData, $scope.$on("$stateChangeSuccess", function(event, toState) {
...
} ]).controller("AuthCtrl", [ "$rootScope", "$scope", "$sce", "Authorise", "$state", function($rootScope, $scope, $sce, Authorise) {
    $scope.$on("authEvent", function(event, broadcast) {
        $scope.show_loader = !1, $scope.logged_in = broadcast.status, $scope.reason = broadcast.msg;
    }), $scope.$on("isAuthorised", function(event, broadcast) {
        var auth_status = broadcast.status;
        if ($scope.show_loader = auth_status, $scope.logged_in = auth_status, auth_status) $scope.user = broadcast.msg;
    }), $scope.$on("passwordUtil", function(event, broadcast) {
        $scope.show_loader = !1, $scope.reason = $sce.trustAsHtml(broadcast.msg);
    }), $scope.checkLoginStatus = function() {
        Authorise.check_login_status();
    }, $scope.loginUser = function() {
        $scope.show_loader = !0, Authorise.login($scope.auth.email, $scope.auth.password);
    }, $scope.logoutUser = function() {
        Authorise.logout(), $scope.show_loader = !1, $scope.logged_in = !1;
    }, $scope.retrievePassword = function() {
        $scope.show_loader = !0, Authorise.retrieve_password($scope.auth.email);
    }, $scope.changePassword = function() {
        $scope.show_loader = !0, Authorise.change_password($scope.auth.email, $scope.auth.old_password, $scope.auth.new_password);
    };
} ]);