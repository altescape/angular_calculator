!function(a, b) {
    ...
    !function() {
        ...
        if (f.style) f.style.backgroundClip = "content-box", f.cloneNode(!0).style.backgroundClip = "", 
    ...
    n.fn.extend({
        ...
        attr: function(a, b, c) {
            ...
            c) : void n.removeAttr(a, b)); else return void 0;
        },
    ...
    n.fn.extend({
        ...
        prop: function(a, b, c) {
            ...
            e = n.propHooks[b]), void 0 !== c ? e && "set" in e && void 0 !== (d = e.set(a, c, b)) ? d : a[b] = c : e && "get" in e && null !== (d = e.get(a, b)) ? d : a[b]; else return void 0;
        },
        propHooks: {
            tabIndex: {
                get: function(a) {
    ...
    n.fn.extend({
        val: function(a) {
            ...
            "string" == typeof c ? c.replace(bc, "") : null == c ? "" : c); else return void 0;
        }
    }), n.extend({
        valHooks: {
            option: {
    ...
    function minErr(module, ErrorConstructor) {
        return ErrorConstructor = ErrorConstructor || Error, function() {
            ...
            for (message = prefix + template.replace(/\{\d+\}/g, function(match) {
                ...
                if (index + 2 < templateArgs.length) return toDebugString(templateArgs[index + 2]); else return match;
    ...
    function isArrayLike(obj) {
        ...
        if (obj.nodeType === NODE_TYPE_ELEMENT && length) return !0; else return isString(obj) || isArray(obj) || 0 === length || "number" == typeof length && length > 0 && length - 1 in obj;
    }
    function forEach(obj, iterator, context) {
        var key, length;
        if (obj) if (isFunction(obj)) {
            for (key in obj) if ("prototype" != key && "length" != key && "name" != key && (!obj.hasOwnProperty || obj.hasOwnProperty(key))) iterator.call(context, obj[key], key, obj);
        } else if (isArray(obj) || isArrayLike(obj)) {
            var isPrimitive = "object" != typeof obj;
            for (key = 0, length = obj.length; length > key; key++) if (isPrimitive || key in obj) iterator.call(context, obj[key], key, obj);
        } else if (obj.forEach && obj.forEach !== forEach) obj.forEach(iterator, context, obj); else for (key in obj) if (obj.hasOwnProperty(key)) iterator.call(context, obj[key], key, obj);
        return obj;
    }
    ...
    function copy(source, destination, stackSource, stackDest) {
        ...
        if (!destination) {
            if (destination = source, source) if (isArray(source)) destination = copy(source, [], stackSource, stackDest); else if (isDate(source)) destination = new Date(source.getTime()); else if (isRegExp(source)) destination = new RegExp(source.source, source.toString().match(/[^\/]*$/)[0]), 
            ...
            if (isArray(source)) {
                ...
                for (var i = 0; i < source.length; i++) {
                    if (result = copy(source[i], null, stackSource, stackDest), isObject(source[i])) stackSource.push(source[i]), 
                    stackDest.push(result);
                    destination.push(result);
                }
            } else {
                var h = destination.$$hashKey;
                if (isArray(destination)) destination.length = 0; else forEach(destination, function(value, key) {
                    delete destination[key];
                });
                for (var key in source) if (source.hasOwnProperty(key)) {
                    if (result = copy(source[key], null, stackSource, stackDest), isObject(source[key])) stackSource.push(source[key]), 
                    stackDest.push(result);
    ...
    function shallowCopy(src, dst) {
        if (isArray(src)) {
            ...
            dst = dst || {};
            for (var key in src) if ("$" !== key.charAt(0) || "$" !== key.charAt(1)) dst[key] = src[key];
        }
        return dst || src;
    }
    function equals(o1, o2) {
        ...
        if (t1 == t2) if ("object" == t1) if (isArray(o1)) {
            if (!isArray(o2)) return !1;
            if ((length = o1.length) == o2.length) {
                for (key = 0; length > key; key++) if (!equals(o1[key], o2[key])) return !1;
                return !0;
            }
        } else if (isDate(o1)) if (!isDate(o2)) return !1; else return equals(o1.getTime(), o2.getTime()); else if (isRegExp(o1) && isRegExp(o2)) return o1.toString() == o2.toString(); else {
    ...
    function bind(self, fn) {
        ...
        if (isFunction(fn) && !(fn instanceof RegExp)) return curryArgs.length ? function() {
    ...
    function toJsonReplacer(key, value) {
        var val = value;
        if ("string" == typeof key && "$" === key.charAt(0) && "$" === key.charAt(1)) val = undefined; else if (isWindow(value)) val = "$WINDOW"; else if (value && document === value) val = "$DOCUMENT"; else if (isScope(value)) val = "$SCOPE";
        return val;
    }
    ...
    function parseKeyValue(keyValue) {
        ...
        return forEach((keyValue || "").split("&"), function(keyValue) {
            if (keyValue) if (key_value = keyValue.replace(/\+/g, "%20").split("="), key = tryDecodeURIComponent(key_value[0]), 
            isDefined(key)) {
                var val = isDefined(key_value[1]) ? tryDecodeURIComponent(key_value[1]) : !0;
                if (!hasOwnProperty.call(obj, key)) obj[key] = val; else if (isArray(obj[key])) obj[key].push(val); else obj[key] = [ obj[key], val ];
            }
        }), obj;
    }
    function toKeyValue(obj) {
        ...
        return forEach(obj, function(value, key) {
            if (isArray(value)) forEach(value, function(arrayValue) {
                parts.push(encodeUriQuery(key, !0) + (arrayValue === !0 ? "" : "=" + encodeUriQuery(arrayValue, !0)));
            }); else parts.push(encodeUriQuery(key, !0) + (value === !0 ? "" : "=" + encodeUriQuery(value, !0)));
    ...
    function angularInit(element, bootstrap) {
        ...
        if (forEach(ngAttrPrefixes, function(prefix) {
            var name = prefix + "app";
            if (!appElement && element.hasAttribute && element.hasAttribute(name)) appElement = element, 
            module = element.getAttribute(name);
        }), forEach(ngAttrPrefixes, function(prefix) {
            var candidate, name = prefix + "app";
            if (!appElement && (candidate = element.querySelector("[" + name.replace(":", "\\:") + "]"))) appElement = candidate, 
    ...
    function bootstrap(element, modules, config) {
        ...
        var doBootstrap = function() {
            ...
            if (modules = modules || [], modules.unshift([ "$provide", function($provide) {
        ...
        if (window.name = window.name.replace(NG_DEFER_BOOTSTRAP, ""), angular.resumeBootstrap = function(extraModules) {
    ...
    function bindJQuery() {
        ...
        if (!bindJQueryFired) {
            if (jQuery = window.jQuery, jQuery && jQuery.fn.on) jqLite = jQuery, extend(jQuery.fn, {
                ...
                if (!skipDestroyOnNextJQueryCleanData) {
                    for (var elem, i = 0; null != (elem = elems[i]); i++) if (events = jQuery._data(elem, "events"), 
                    events && events.$destroy) jQuery(elem).triggerHandler("$destroy");
                } else skipDestroyOnNextJQueryCleanData = !1;
    ...
    function assertArgFn(arg, name, acceptArrayAnnotation) {
        ...
        return assertArg(isFunction(arg), name, "not a function, got " + (arg && "object" == typeof arg ? arg.constructor.name || "Object" : typeof arg)), 
        arg;
    }
    ...
    function getter(obj, path, bindFnToScope) {
        if (!path) return obj;
        for (var key, keys = path.split("."), lastInstance = obj, len = keys.length, i = 0; len > i; i++) if (key = keys[i], 
        obj) obj = (lastInstance = obj)[key];
        if (!bindFnToScope && isFunction(obj)) return bind(lastInstance, obj); else return obj;
    }
    ...
    function setupModuleLoader(window) {
        ...
        return angular.$$minErr = angular.$$minErr || minErr, ensure(angular, "module", function() {
            ...
            return function(name, requires, configFn) {
                ...
                if (assertNotHasOwnProperty(name, "module"), requires && modules.hasOwnProperty(name)) modules[name] = null;
    ...
    function toDebugString(obj) {
        if ("function" == typeof obj) return obj.toString().replace(/ \{[\s\S]*$/, ""); else if ("undefined" == typeof obj) return "undefined"; else if ("string" != typeof obj) return serializeObject(obj);
        return obj;
    }
    ...
    function jqLiteParseHTML(html, context) {
        ...
        if (parsed = jqLiteBuildFragment(html, context)) return parsed.childNodes; else return [];
    }
    function JQLite(element) {
        if (element instanceof JQLite) return element;
        var argIsString;
        if (isString(element)) element = trim(element), argIsString = !0;
    ...
    function jqLiteDealoc(element, onlyDescendants) {
        ...
        if (element.querySelectorAll) for (var descendants = element.querySelectorAll("*"), i = 0, l = descendants.length; l > i; i++) jqLiteRemoveData(descendants[i]);
    }
    function jqLiteOff(element, type, fn, unsupported) {
        ...
        if (handle) if (!type) for (type in events) {
            if ("$destroy" !== type) removeEventListenerFn(element, type, handle);
            delete events[type];
        } else forEach(type.split(" "), function(type) {
            if (isDefined(fn)) {
    ...
    function jqLiteRemoveData(element, name) {
        ...
        if (expandoStore) {
            ...
            if (expandoStore.handle) {
                if (expandoStore.events.$destroy) expandoStore.handle({}, "$destroy");
                jqLiteOff(element);
            }
    ...
    function jqLiteExpandoStore(element, createIfNecessary) {
        var expandoId = element.ng339, expandoStore = expandoId && jqCache[expandoId];
        if (createIfNecessary && !expandoStore) element.ng339 = expandoId = jqNextId(), 
    ...
    function jqLiteHasClass(element, selector) {
        if (!element.getAttribute) return !1; else return (" " + (element.getAttribute("class") || "") + " ").replace(/[\n\t]/g, " ").indexOf(" " + selector + " ") > -1;
    }
    function jqLiteRemoveClass(element, cssClasses) {
        if (cssClasses && element.setAttribute) forEach(cssClasses.split(" "), function(cssClass) {
    ...
    function jqLiteAddClass(element, cssClasses) {
        if (cssClasses && element.setAttribute) {
            ...
            forEach(cssClasses.split(" "), function(cssClass) {
                if (cssClass = trim(cssClass), -1 === existingClasses.indexOf(" " + cssClass + " ")) existingClasses += cssClass + " ";
    ...
    function jqLiteInheritedData(element, name, value) {
        if (element.nodeType == NODE_TYPE_DOCUMENT) element = element.documentElement;
    ...
    function jqLiteDocumentLoaded(action, win) {
        if (win = win || window, "complete" === win.document.readyState) win.setTimeout(action); else jqLite(win).on("load", action);
    }
            ...
            if (eventFnsLength) {
                if (isUndefined(event.immediatePropagationStopped)) {
                    ...
                    event.stopImmediatePropagation = function() {
                        if (event.immediatePropagationStopped = !0, event.stopPropagation) event.stopPropagation();
                        if (originalStopImmediatePropagation) originalStopImmediatePropagation.call(event);
                    };
                }
                if (event.isImmediatePropagationStopped = function() {
                    return event.immediatePropagationStopped === !0;
                }, eventFnsLength > 1) eventFns = shallowCopy(eventFns);
                for (var i = 0; eventFnsLength > i; i++) if (!event.isImmediatePropagationStopped()) eventFns[i].call(element, event);
            }
        };
    ...
    function hashKey(obj, nextUidFn) {
        ...
        if (key) {
            if ("function" == typeof key) key = obj.$$hashKey();
            return key;
        }
        var objType = typeof obj;
        if ("function" == objType || "object" == objType && null !== obj) key = obj.$$hashKey = objType + ":" + (nextUidFn || nextUid)(); else key = objType + ":" + obj;
        return key;
    }
    ...
    function annotate(fn, strictDi, name) {
        ...
        if ("function" == typeof fn) {
            if (!($inject = fn.$inject)) {
                if ($inject = [], fn.length) {
                    if (strictDi) {
                        if (!isString(name) || !name) name = fn.name || anonFn(fn);
        ...
        } else if (isArray(fn)) last = fn.length - 1, assertArgFn(fn[last], "fn"), $inject = fn.slice(0, last); else assertArgFn(fn, "fn", !0);
        return $inject;
    }
    function createInjector(modulesToLoad, strictDi) {
        function supportObject(delegate) {
            return function(key, value) {
                if (isObject(key)) forEach(key, reverseParams(delegate)); else return delegate(key, value);
            };
        }
        ...
        function loadModules(modulesToLoad) {
            ...
            return forEach(modulesToLoad, function(module) {
                ...
                if (!loadedModules.get(module)) {
                    ...
                    try {
                        if (isString(module)) moduleFn = angularModule(module), runBlocks = runBlocks.concat(loadModules(moduleFn.requires)).concat(moduleFn._runBlocks), 
                        runInvokeQueue(moduleFn._invokeQueue), runInvokeQueue(moduleFn._configBlocks); else if (isFunction(module)) runBlocks.push(providerInjector.invoke(module)); else if (isArray(module)) runBlocks.push(providerInjector.invoke(module)); else assertArgFn(module, "module");
                    } catch (e) {
                        if (isArray(module)) module = module[module.length - 1];
                        if (e.message && e.stack && -1 == e.stack.indexOf(e.message)) e = e.message + "\n" + e.stack;
        ...
        function createInternalInjector(cache, factory) {
            function getService(serviceName, caller) {
                if (cache.hasOwnProperty(serviceName)) {
                    ...
                    if (cache[serviceName] === INSTANTIATING) delete cache[serviceName];
                    throw err;
                } finally {
                    path.shift();
                }
            ...
            function invoke(fn, self, locals, serviceName) {
                if ("string" == typeof locals) serviceName = locals, locals = null;
        ...
        this.disableAutoScrolling = function() {
            ...
            function getFirstAnchor(list) {
                ...
                return Array.prototype.some.call(list, function(element) {
                    if ("a" === nodeName_(element)) return result = element, !0; else return void 0;
                }), result;
            }
            function getYOffset() {
                ...
                if (isFunction(offset)) offset = offset(); else if (isElement(offset)) {
                    ...
                    if ("fixed" !== style.position) offset = 0; else offset = elem.getBoundingClientRect().bottom;
                } else if (!isNumber(offset)) offset = 0;
                return offset;
            }
            ...
            function scroll() {
                ...
                if (!hash) scrollTo(null); else if (elm = document.getElementById(hash)) scrollTo(elm); else if (elm = getFirstAnchor(document.getElementsByName(hash))) scrollTo(elm); else if ("top" === hash) scrollTo(null);
            }
            var document = $window.document;
            if (autoScrollingEnabled) $rootScope.$watch(function() {
                return $location.hash();
            }, function(newVal, oldVal) {
                if (newVal !== oldVal || "" !== newVal) jqLiteDocumentLoaded(function() {
    ...
    function Browser(window, document, $log, $sniffer) {
        ...
        function cacheState() {
            if (cachedState = window.history.state, cachedState = isUndefined(cachedState) ? null : cachedState, 
            equals(cachedState, lastCachedState)) cachedState = lastCachedState;
            lastCachedState = cachedState;
        }
        function fireUrlChange() {
            if (lastBrowserUrl !== self.url() || lastHistoryState !== cachedState) lastBrowserUrl = self.url(), 
        ...
        self.$$completeOutstandingRequest = completeOutstandingRequest, self.$$incOutstandingRequestCount = function() {
            ...
            if (forEach(pollFns, function(pollFn) {
                pollFn();
            }), 0 === outstandingRequestCount) callback(); else outstandingRequestCallbacks.push(callback);
        };
        var pollTimeout, pollFns = [];
        self.addPollFn = function(fn) {
            if (isUndefined(pollTimeout)) startPoller(100, setTimeout);
            return pollFns.push(fn), fn;
        };
        ...
        cacheState(), lastHistoryState = cachedState, self.url = function(url, replace, state) {
            ...
            if (url) {
                ...
                if (lastBrowserUrl = url, lastHistoryState = state, $sniffer.history && (!sameBase || !sameState)) history[replace ? "replaceState" : "pushState"](state, "", url), 
                cacheState(), lastHistoryState = cachedState; else {
                    if (!sameBase) reloadLocation = url;
                    if (replace) location.replace(url); else if (!sameBase) location.href = url; else location.hash = getHash(url);
                }
                return self;
            } else return reloadLocation || location.href.replace(/%27/g, "'");
        }, self.state = function() {
            return cachedState;
        };
        ...
        self.cookies = function(name, value) {
            ...
            if (name) {
                if (value === undefined) rawDocument.cookie = encodeURIComponent(name) + "=;path=" + cookiePath + ";expires=Thu, 01 Jan 1970 00:00:00 GMT"; else if (isString(value)) if (cookieLength = (rawDocument.cookie = encodeURIComponent(name) + "=" + encodeURIComponent(value) + ";path=" + cookiePath).length + 1, 
                ...
                index = cookie.indexOf("="), index > 0) if (name = safeDecodeURIComponent(cookie.substring(0, index)), 
            ...
            if (pendingDeferIds[deferId]) return delete pendingDeferIds[deferId], clearTimeout(deferId), 
            completeOutstandingRequest(noop), !0; else return !1;
        };
    }
        ...
        this.$get = function() {
            function cacheFactory(cacheId, options) {
                function refresh(entry) {
                    if (entry != freshEnd) {
                        if (!staleEnd) staleEnd = entry; else if (staleEnd == entry) staleEnd = entry.n;
                ...
                return caches[cacheId] = {
                    put: function(key, value) {
                        ...
                        if (!isUndefined(value)) {
                            if (!(key in data)) size++;
                            if (data[key] = value, size > capacity) this.remove(staleEnd.key);
                            return value;
                        }
                    },
                    get: function(key) {
    ...
    function $CompileProvider($provide, $$sanitizeUriProvider) {
        ...
        this.directive = function registerDirective(name, directiveFactory) {
            if (assertNotHasOwnProperty(name, "directive"), isString(name)) {
                if (assertArg(directiveFactory, "directiveFactory"), !hasDirectives.hasOwnProperty(name)) hasDirectives[name] = [], 
                $provide.factory(name + Suffix, [ "$injector", "$exceptionHandler", function($injector, $exceptionHandler) {
                    ...
                    return forEach(hasDirectives[name], function(directiveFactory, index) {
                        try {
                            ...
                            }; else if (!directive.compile && directive.link) directive.compile = valueFn(directive.link);
                            if (directive.priority = directive.priority || 0, directive.index = index, directive.name = directive.name || name, 
            ...
            if (isDefined(regexp)) return $$sanitizeUriProvider.aHrefSanitizationWhitelist(regexp), 
            this; else return $$sanitizeUriProvider.aHrefSanitizationWhitelist();
        }, this.imgSrcSanitizationWhitelist = function(regexp) {
            if (isDefined(regexp)) return $$sanitizeUriProvider.imgSrcSanitizationWhitelist(regexp), 
            this; else return $$sanitizeUriProvider.imgSrcSanitizationWhitelist();
        };
        var debugInfoEnabled = !0;
        this.debugInfoEnabled = function(enabled) {
            if (isDefined(enabled)) return debugInfoEnabled = enabled, this; else return debugInfoEnabled;
            ...
            function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
                ...
                forEach($compileNodes, function(node, index) {
                    if (node.nodeType == NODE_TYPE_TEXT && node.nodeValue.match(/\S+/)) $compileNodes[index] = jqLite(node).wrap("<span></span>").parent()[0];
                });
                ...
                return function(scope, cloneConnectFn, options) {
                    ...
                    var $linkNode;
                    if ("html" !== namespace) $linkNode = jqLite(wrapTemplate(namespace, jqLite("<div>").append($compileNodes).html())); else if (cloneConnectFn) $linkNode = JQLitePrototype.clone.call($compileNodes); else $linkNode = $compileNodes;
                    if (transcludeControllers) for (var controllerName in transcludeControllers) $linkNode.data("$" + controllerName + "Controller", transcludeControllers[controllerName].instance);
                    if (compile.$$addScopeInfo($linkNode, scope), cloneConnectFn) cloneConnectFn($linkNode, scope);
            ...
            function detectNamespaceForChildElements(parentElement) {
                ...
                if (!node) return "html"; else return "foreignobject" !== nodeName_(node) && node.toString().match(/SVG/) ? "svg" : "html";
            }
            function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
                function compositeLinkFn(scope, nodeList, $rootElement, parentBoundTranscludeFn) {
                    ...
                    nodeLinkFn = linkFns[i++], childLinkFn = linkFns[i++], nodeLinkFn) {
                        if (nodeLinkFn.scope) childScope = scope.$new(), compile.$$addScopeInfo(jqLite(node), childScope); else childScope = scope;
                ...
                for (var attrs, directives, nodeLinkFn, childNodes, childLinkFn, linkFnFound, nodeLinkFnFound, linkFns = [], i = 0; i < nodeList.length; i++) {
                    if (attrs = new Attributes(), directives = collectDirectives(nodeList[i], [], attrs, 0 === i ? maxPriority : undefined, ignoreDirective), 
                    nodeLinkFn = directives.length ? applyDirectivesToNode(directives, nodeList[i], attrs, transcludeFn, $rootElement, null, [], [], previousCompileContext) : null, 
                    nodeLinkFn && nodeLinkFn.scope) compile.$$addScopeClass(attrs.$$element);
                    if (childLinkFn = nodeLinkFn && nodeLinkFn.terminal || !(childNodes = nodeList[i].childNodes) || !childNodes.length ? null : compileNodes(childNodes, nodeLinkFn ? (nodeLinkFn.transcludeOnThisElement || !nodeLinkFn.templateOnThisElement) && nodeLinkFn.transclude : transcludeFn), 
            ...
            function createBoundTranscludeFn(scope, transcludeFn, previousBoundTranscludeFn) {
                var boundTranscludeFn = function(transcludedScope, cloneFn, controllers, futureParentElement, containingScope) {
                    if (!transcludedScope) transcludedScope = scope.$new(!1, containingScope), transcludedScope.$$transcluded = !0;
                ...
                switch (nodeType) {
                  case NODE_TYPE_ELEMENT:
                    ...
                    for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; jj > j; j++) {
                        ...
                        if (attr = nAttrs[j], name = attr.name, value = trim(attr.value), ngAttrName = directiveNormalize(name), 
                        ...
                        if (directiveIsMultiElement(directiveNName)) if (ngAttrName === directiveNName + "Start") attrStartName = name, 
                        attrEndName = name.substr(0, name.length - 5) + "end", name = name.substr(0, name.length - 6);
                        if (nName = directiveNormalize(name.toLowerCase()), attrsMap[nName] = name, isNgAttr || !attrs.hasOwnProperty(nName)) if (attrs[nName] = value, 
                    ...
                    if (isString(className) && "" !== className) for (;match = CLASS_DIRECTIVE_REGEXP.exec(className); ) {
                        if (nName = directiveNormalize(match[2]), addDirective(directives, nName, "C", maxPriority, ignoreDirective)) attrs[nName] = trim(match[3]);
                  ...
                  case NODE_TYPE_COMMENT:
                    try {
                        if (match = COMMENT_DIRECTIVE_REGEXP.exec(node.nodeValue)) if (nName = directiveNormalize(match[1]), 
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                function addLinkFns(pre, post, attrStart, attrEnd) {
                    if (pre) {
                        ...
                        if (pre.require = directive.require, pre.directiveName = directiveName, newIsolateScopeDirective === directive || directive.$$isolateScope) pre = cloneAndAnnotateFn(pre, {
                            isolateScope: !0
                        });
                        preLinkFns.push(pre);
                    }
                    if (post) {
                        ...
                        if (post.require = directive.require, post.directiveName = directiveName, newIsolateScopeDirective === directive || directive.$$isolateScope) post = cloneAndAnnotateFn(post, {
                            isolateScope: !0
                        });
                ...
                function getControllers(directiveName, require, $element, elementControllers) {
                    ...
                    if (isString(require)) {
                        ...
                        match[3]) if (match[1]) match[3] = null; else match[1] = match[3];
                        if ("^" === match[1]) retrievalMethod = "inheritedData"; else if ("^^" === match[1]) retrievalMethod = "inheritedData", 
                        $searchElement = $element.parent();
                        if ("?" === match[2]) optional = !0;
                        if (value = null, elementControllers && "data" === retrievalMethod) if (value = elementControllers[require]) value = value.instance;
                        if (value = value || $searchElement[retrievalMethod]("$" + require + "Controller"), 
                        !value && !optional) throw $compileMinErr("ctreq", "Controller '{0}', required by directive '{1}', can't be found!", require, directiveName);
                        return value || null;
                    } else if (isArray(require)) value = [], forEach(require, function(require) {
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    function controllersBoundTransclude(scope, cloneAttachFn, futureParentElement) {
                        ...
                        if (!isScope(scope)) futureParentElement = cloneAttachFn, cloneAttachFn = scope, 
                        scope = undefined;
                    ...
                    if (compileNode === linkNode) attrs = templateAttrs, $element = templateAttrs.$$element; else $element = jqLite(linkNode), 
                    ...
                    if (controllerDirectives) controllers = {}, elementControllers = {}, forEach(controllerDirectives, function(directive) {
                        ...
                        if (controller = directive.controller, "@" == controller) controller = attrs[directive.name];
                        if (controllerInstance = $controller(controller, locals, !0, directive.controllerAs), 
                    ...
                    if (newIsolateScopeDirective) {
                        ...
                        if (isolateScopeController && isolateScopeController.identifier && newIsolateScopeDirective.bindToController === !0) isolateBindingContext = isolateScopeController.instance;
                        forEach(isolateScope.$$isolateBindings = newIsolateScopeDirective.$$isolateBindings, function(definition, scopeName) {
                            ...
                            switch (mode) {
                              case "@":
                                if (attrs.$observe(attrName, function(value) {
                              ...
                              case "=":
                                ...
                                if (parentGet = $parse(attrs[attrName]), parentGet.literal) compare = equals; else compare = function(a, b) {
                                    return a === b || a !== a && b !== b;
                                };
                                ...
                                var parentValueWatch = function(parentValue) {
                                    if (!compare(parentValue, isolateBindingContext[scopeName])) if (!compare(parentValue, lastValue)) isolateBindingContext[scopeName] = parentValue; else parentSet(scope, parentValue = isolateBindingContext[scopeName]);
                                    return lastValue = parentValue;
                                };
                                parentValueWatch.$stateful = !0;
                                var unwatch;
                                if (definition.collection) unwatch = scope.$watchCollection(attrs[attrName], parentValueWatch); else unwatch = scope.$watch($parse(attrs[attrName], parentValueWatch), null, parentGet.literal);
                    ...
                    if (controllers) forEach(controllers, function(controller) {
                        controller();
                    }), controllers = null;
                    ...
                    if (newIsolateScopeDirective && (newIsolateScopeDirective.template || null === newIsolateScopeDirective.templateUrl)) scopeToChild = isolateScope;
                    for (childLinkFn && childLinkFn(scopeToChild, linkNode.childNodes, undefined, boundTranscludeFn), 
                ...
                for (var newScopeDirective, controllers, directive, directiveName, $template, linkFn, directiveValue, terminalPriority = -Number.MAX_VALUE, controllerDirectives = previousCompileContext.controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasTemplate = !1, hasElementTranscludeDirective = previousCompileContext.hasElementTranscludeDirective, $compileNode = templateAttrs.$$element = jqLite(compileNode), replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, i = 0, ii = directives.length; ii > i; i++) {
                    ...
                    if ($template = undefined, terminalPriority > directive.priority) break;
                    if (directiveValue = directive.scope) {
                        if (!directive.templateUrl) if (isObject(directiveValue)) assertNoDuplicate("new/isolated scope", newIsolateScopeDirective || newScopeDirective, directive, $compileNode), 
                        newIsolateScopeDirective = directive; else assertNoDuplicate("new/isolated scope", newIsolateScopeDirective, directive, $compileNode);
                        newScopeDirective = newScopeDirective || directive;
                    }
                    if (directiveName = directive.name, !directive.templateUrl && directive.controller) directiveValue = directive.controller, 
                    ...
                    if (directiveValue = directive.transclude) {
                        if (hasTranscludeDirective = !0, !directive.$$tlb) assertNoDuplicate("transclusion", nonTlbTranscludeDirective, directive, $compileNode), 
                        nonTlbTranscludeDirective = directive;
                        if ("element" == directiveValue) hasElementTranscludeDirective = !0, terminalPriority = directive.priority, 
                        $template = $compileNode, $compileNode = templateAttrs.$$element = jqLite(document.createComment(" " + directiveName + ": " + templateAttrs[directiveName] + " ")), 
                        compileNode = $compileNode[0], replaceWith(jqCollection, sliceArgs($template), compileNode), 
                        childTranscludeFn = compile($template, transcludeFn, terminalPriority, replaceDirective && replaceDirective.name, {
                            nonTlbTranscludeDirective: nonTlbTranscludeDirective
                        }); else $template = jqLite(jqLiteClone(compileNode)).contents(), $compileNode.empty(), 
                    ...
                    directiveValue = denormalizeTemplate(directiveValue), directive.replace) {
                        if (replaceDirective = directive, jqLiteIsTextNode(directiveValue)) $template = []; else $template = removeComments(wrapTemplate(directive.templateNamespace, trim(directiveValue)));
                        if (compileNode = $template[0], 1 != $template.length || compileNode.nodeType !== NODE_TYPE_ELEMENT) throw $compileMinErr("tplrt", "Template for directive '{0}' must have exactly one root element. {1}", directiveName, "");
                        replaceWith(jqCollection, $compileNode, compileNode);
                        var newTemplateAttrs = {
                            $attr: {}
                        }, templateDirectives = collectDirectives(compileNode, [], newTemplateAttrs), unprocessedDirectives = directives.splice(i + 1, directives.length - (i + 1));
                        if (newIsolateScopeDirective) markDirectivesAsIsolate(templateDirectives);
                        directives = directives.concat(templateDirectives).concat(unprocessedDirectives), 
                        mergeTemplateAttributes(templateAttrs, newTemplateAttrs), ii = directives.length;
                    } else $compileNode.html(directiveValue);
                    if (directive.templateUrl) {
                        if (hasTemplate = !0, assertNoDuplicate("template", templateDirective, directive, $compileNode), 
                        ...
                        if (linkFn = directive.compile($compileNode, templateAttrs, childTranscludeFn), 
                        isFunction(linkFn)) addLinkFns(null, linkFn, attrStart, attrEnd); else if (linkFn) addLinkFns(linkFn.pre, linkFn.post, attrStart, attrEnd);
                    } catch (e) {
            ...
            function addDirective(tDirectives, name, location, maxPriority, ignoreDirective, startAttrName, endAttrName) {
                ...
                if (hasDirectives.hasOwnProperty(name)) for (var directive, directives = $injector.get(name + Suffix), i = 0, ii = directives.length; ii > i; i++) try {
                    if (directive = directives[i], (maxPriority === undefined || maxPriority > directive.priority) && -1 != directive.restrict.indexOf(location)) {
            ...
            function mergeTemplateAttributes(dst, src) {
                ...
                forEach(dst, function(value, key) {
                    if ("$" != key.charAt(0)) {
                        if (src[key] && src[key] !== value) value += ("style" === key ? ";" : " ") + src[key];
                        dst.$set(key, value, !0, srcAttr[key]);
                    }
                }), forEach(src, function(value, key) {
                    if ("class" == key) safeAddClass($element, value), dst["class"] = (dst["class"] ? dst["class"] + " " : "") + value; else if ("style" == key) $element.attr("style", $element.attr("style") + ";" + value), 
                    dst.style = (dst.style ? dst.style + ";" : "") + value; else if ("$" != key.charAt(0) && !dst.hasOwnProperty(key)) dst[key] = value, 
            ...
            function compileTemplateUrl(directives, $compileNode, tAttrs, $rootElement, childTranscludeFn, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                return $compileNode.empty(), $templateRequest($sce.getTrustedResourceUrl(templateUrl)).then(function(content) {
                    ...
                    if (content = denormalizeTemplate(content), origAsyncDirective.replace) {
                        if (jqLiteIsTextNode(content)) $template = []; else $template = removeComments(wrapTemplate(templateNamespace, trim(content)));
                        if (compileNode = $template[0], 1 != $template.length || compileNode.nodeType !== NODE_TYPE_ELEMENT) throw $compileMinErr("tplrt", "Template for directive '{0}' must have exactly one root element. {1}", origAsyncDirective.name, templateUrl);
                        tempTemplateAttrs = {
                            $attr: {}
                        }, replaceWith($rootElement, $compileNode, compileNode);
                        var templateDirectives = collectDirectives(compileNode, [], tempTemplateAttrs);
                        if (isObject(origAsyncDirective.scope)) markDirectivesAsIsolate(templateDirectives);
                    ...
                    forEach($rootElement, function(node, i) {
                        if (node == compileNode) $rootElement[i] = $compileNode[0];
                    }), afterTemplateChildLinkFn = compileNodes($compileNode[0].childNodes, childTranscludeFn); linkQueue.length; ) {
                        var scope = linkQueue.shift(), beforeTemplateLinkNode = linkQueue.shift(), linkRootElement = linkQueue.shift(), boundTranscludeFn = linkQueue.shift(), linkNode = $compileNode[0];
                        if (!scope.$$destroyed) {
                            if (beforeTemplateLinkNode !== beforeTemplateCompileNode) {
                                var oldClasses = beforeTemplateLinkNode.className;
                                if (!previousCompileContext.hasElementTranscludeDirective || !origAsyncDirective.replace) linkNode = jqLiteClone(compileNode);
                    ...
                    if (!scope.$$destroyed) if (linkQueue) linkQueue.push(scope, node, rootElement, childBoundTranscludeFn); else {
                        if (afterTemplateNodeLinkFn.transcludeOnThisElement) childBoundTranscludeFn = createBoundTranscludeFn(scope, afterTemplateNodeLinkFn.transclude, boundTranscludeFn);
            ...
            function byPriority(a, b) {
                ...
                if (a.name !== b.name) return a.name < b.name ? -1 : 1; else return a.index - b.index;
            }
            ...
            function addTextInterpolateDirective(directives, text) {
                ...
                if (interpolateFn) directives.push({
                    priority: 0,
                    compile: function(templateNode) {
                        ...
                        return function(scope, node) {
                            ...
                            compile.$$addBindingInfo(parent, interpolateFn.expressions), scope.$watch(interpolateFn, function(value) {
            ...
            function getTrustedContext(node, attrNormalizedName) {
                ...
                if ("xlinkHref" == attrNormalizedName || "form" == tag && "action" == attrNormalizedName || "img" != tag && ("src" == attrNormalizedName || "ngSrc" == attrNormalizedName)) return $sce.RESOURCE_URL; else return void 0;
            }
                        ...
                        compile: function() {
                            return {
                                pre: function(scope, element, attr) {
                                    ...
                                    if (newValue !== value) interpolateFn = newValue && $interpolate(newValue, !0, trustedContext, allOrNothing), 
                                    value = newValue;
                                    if (interpolateFn) attr[name] = interpolateFn(scope), ($$observers[name] || ($$observers[name] = [])).$$inter = !0, 
                                    (attr.$$observers && attr.$$observers[name].$$scope || scope).$watch(interpolateFn, function(newValue, oldValue) {
                                        if ("class" === name && newValue != oldValue) attr.$updateClass(newValue, oldValue); else attr.$set(name, newValue);
                                    });
                                }
                            };
                        }
            ...
            function replaceWith($rootElement, elementsToRemove, newNode) {
                ...
                if ($rootElement) for (i = 0, ii = $rootElement.length; ii > i; i++) if ($rootElement[i] == firstElementToRemove) {
                    ...
                    j2++) if (jj > j2) $rootElement[j] = $rootElement[j2]; else delete $rootElement[j];
                    if ($rootElement.length -= removeCount - 1, $rootElement.context === firstElementToRemove) $rootElement.context = newNode;
                    break;
                }
                if (parent) parent.replaceChild(newNode, firstElementToRemove);
                var fragment = document.createDocumentFragment();
                if (fragment.appendChild(firstElementToRemove), jqLite(newNode).data(jqLite(firstElementToRemove).data()), 
                !jQuery) delete jqLite.cache[firstElementToRemove[jqLite.expando]]; else skipDestroyOnNextJQueryCleanData = !0, 
            ...
            Attributes.prototype = {
                ...
                $addClass: function(classVal) {
                    if (classVal && classVal.length > 0) $animate.addClass(this.$$element, classVal);
                },
                $removeClass: function(classVal) {
                    if (classVal && classVal.length > 0) $animate.removeClass(this.$$element, classVal);
                },
                $updateClass: function(newClasses, oldClasses) {
                    var toAdd = tokenDifference(newClasses, oldClasses);
                    if (toAdd && toAdd.length) $animate.addClass(this.$$element, toAdd);
                    var toRemove = tokenDifference(oldClasses, newClasses);
                    if (toRemove && toRemove.length) $animate.removeClass(this.$$element, toRemove);
                },
                $set: function(key, value, writeAttr, attrName) {
                    ...
                    if (booleanKey) this.$$element.prop(key, value), attrName = booleanKey; else if (aliasedKey) this[aliasedKey] = value, 
                    observer = aliasedKey;
                    if (this[key] = value, attrName) this.$attr[key] = attrName; else if (attrName = this.$attr[key], 
                    !attrName) this.$attr[key] = attrName = snake_case(key, "-");
                    if (nodeName = nodeName_(this.$$element), "a" === nodeName && "href" === key || "img" === nodeName && "src" === key) this[key] = value = $$sanitizeUri(value, "src" === key); else if ("img" === nodeName && "srcset" === key) {
                        ...
                        if (result += $$sanitizeUri(trim(lastTuple[0]), !0), 2 === lastTuple.length) result += " " + trim(lastTuple[1]);
                        this[key] = value = result;
                    }
                    if (writeAttr !== !1) if (null === value || value === undefined) this.$$element.removeAttr(attrName); else this.$$element.attr(attrName, value);
                ...
                $observe: function(key, fn) {
                    ...
                    return listeners.push(fn), $rootScope.$evalAsync(function() {
                        if (!listeners.$$inter && attrs.hasOwnProperty(key)) fn(attrs[key]);
                    }), function() {
            ...
            return compile.$$addBindingInfo = debugInfoEnabled ? function($element, binding) {
                ...
                if (isArray(binding)) bindings = bindings.concat(binding); else bindings.push(binding);
    ...
    function removeComments(jqNodes) {
        ...
        for (;i--; ) {
            var node = jqNodes[i];
            if (node.nodeType === NODE_TYPE_COMMENT) splice.call(jqNodes, i, 1);
        }
        return jqNodes;
    }
    function $ControllerProvider() {
        ...
        this.register = function(name, constructor) {
            if (assertNotHasOwnProperty(name, "controller"), isObject(name)) extend(controllers, name); else controllers[name] = constructor;
            ...
            return function(expression, locals, later, ident) {
                ...
                if (later) {
                    ...
                    if (instance = Object.create(controllerPrototype || null), identifier) addIdentifier(locals, identifier, instance, constructor || expression.name);
                    return extend(function() {
                ...
                if (instance = $injector.instantiate(expression, locals, constructor), identifier) addIdentifier(locals, identifier, instance, constructor || expression.name);
                return instance;
            };
        } ];
    }
    ...
    function defaultHttpResponseTransform(data, headers) {
        if (isString(data)) {
            ...
            if (tempData) {
                ...
                if (contentType && 0 === contentType.indexOf(APPLICATION_JSON) || isJsonLike(tempData)) data = fromJson(tempData);
            }
        }
        return data;
    }
    ...
    function parseHeaders(headers) {
        ...
        if (!headers) return parsed; else return forEach(headers.split("\n"), function(line) {
            if (i = line.indexOf(":"), key = lowercase(trim(line.substr(0, i))), val = trim(line.substr(i + 1)), 
    ...
    function transformData(data, headers, status, fns) {
        if (isFunction(fns)) return fns(data, headers, status); else return forEach(fns, function(fn) {
    ...
    function $HttpProvider() {
        ...
        this.useApplyAsync = function(value) {
            if (isDefined(value)) return useApplyAsync = !!value, this; else return useApplyAsync;
        };
        ...
        this.$get = [ "$httpBackend", "$browser", "$cacheFactory", "$rootScope", "$q", "$injector", function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
            function $http(requestConfig) {
                function transformResponse(response) {
                    ...
                    if (!response.data) resp.data = response.data; else resp.data = transformData(response.data, response.headers, response.status, config.transformResponse);
                ...
                function executeHeaderFns(headers) {
                    ...
                    return forEach(headers, function(headerFn, header) {
                        if (isFunction(headerFn)) {
                            if (headerContent = headerFn(), null != headerContent) processedHeaders[header] = headerContent;
                ...
                var serverRequest = function(config) {
                    ...
                    if (isUndefined(reqData)) forEach(headers, function(value, header) {
                        if ("content-type" === lowercase(header)) delete headers[header];
                    });
                    if (isUndefined(config.withCredentials) && !isUndefined(defaults.withCredentials)) config.withCredentials = defaults.withCredentials;
                    return sendReq(config, reqData).then(transformResponse, transformResponse);
                }, chain = [ serverRequest, undefined ], promise = $q.when(config);
                for (forEach(reversedInterceptors, function(interceptor) {
                    if (interceptor.request || interceptor.requestError) chain.unshift(interceptor.request, interceptor.requestError);
                    if (interceptor.response || interceptor.responseError) chain.push(interceptor.response, interceptor.responseError);
                }); chain.length; ) {
            ...
            function sendReq(config, reqData) {
                function done(status, response, headersString, statusText) {
                    ...
                    if (cache) if (isSuccess(status)) cache.put(url, [ status, response, parseHeaders(headersString), statusText ]); else cache.remove(url);
                    if (useApplyAsync) $rootScope.$applyAsync(resolveHttpPromise); else if (resolveHttpPromise(), 
                ...
                function removePendingReq() {
                    ...
                    if (-1 !== idx) $http.pendingRequests.splice(idx, 1);
                }
                ...
                if (cache) if (cachedResp = cache.get(url), isDefined(cachedResp)) if (isPromiseLike(cachedResp)) cachedResp.then(resolvePromiseWithResult, resolvePromiseWithResult); else if (isArray(cachedResp)) resolvePromise(cachedResp[1], cachedResp[0], shallowCopy(cachedResp[2]), cachedResp[3]); else resolvePromise(cachedResp, 200, {}, "OK"); else cache.put(url, promise);
                if (isUndefined(cachedResp)) {
                    var xsrfValue = urlIsSameOrigin(config.url) ? $browser.cookies()[config.xsrfCookieName || defaults.xsrfCookieName] : undefined;
                    if (xsrfValue) reqHeaders[config.xsrfHeaderName || defaults.xsrfHeaderName] = xsrfValue;
            ...
            function buildUrl(url, params) {
                ...
                var parts = [];
                if (forEachSorted(params, function(value, key) {
                    if (null !== value && !isUndefined(value)) {
                        if (!isArray(value)) value = [ value ];
                        forEach(value, function(v) {
                            if (isObject(v)) if (isDate(v)) v = v.toISOString(); else v = toJson(v);
    ...
    function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
        function jsonpReq(url, callbackId, done) {
            ...
            return script.type = "text/javascript", script.src = url, script.async = !0, callback = function(event) {
                ...
                if (event) {
                    if ("load" === event.type && !callbacks[callbackId].called) event = {
                        type: "error"
                    };
                    text = event.type, status = "error" === event.type ? 404 : 200;
                }
                if (done) done(status, text);
        ...
        return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
            ...
            function completeRequest(callback, status, response, headersString, statusText) {
                if (timeoutId !== undefined) $browserDefer.cancel(timeoutId);
                jsonpDone = xhr = null, callback(status, response, headersString, statusText), $browser.$$completeOutstandingRequest(noop);
            }
            if ($browser.$$incOutstandingRequestCount(), url = url || $browser.url(), "jsonp" == lowercase(method)) {
                ...
                xhr.open(method, url, !0), forEach(headers, function(value, key) {
                    if (isDefined(value)) xhr.setRequestHeader(key, value);
                }), xhr.onload = function() {
                    ...
                    if (0 === status) status = response ? 200 : "file" == urlResolve(url).protocol ? 404 : 0;
    ...
    function $InterpolateProvider() {
        ...
        this.startSymbol = function(value) {
            if (value) return startSymbol = value, this; else return startSymbol;
        }, this.endSymbol = function(value) {
            if (value) return endSymbol = value, this; else return endSymbol;
            ...
            function $interpolate(text, mustHaveExpression, trustedContext, allOrNothing) {
                ...
                for (var startIndex, endIndex, exp, index = 0, expressions = [], parseFns = [], textLength = text.length, concat = [], expressionPositions = []; textLength > index; ) if (-1 != (startIndex = text.indexOf(startSymbol, index)) && -1 != (endIndex = text.indexOf(endSymbol, startIndex + startSymbolLength))) {
                    if (index !== startIndex) concat.push(unescapeText(text.substring(index, startIndex)));
                    ...
                    if (index !== textLength) concat.push(unescapeText(text.substring(index)));
                    break;
                }
                    ...
                    return extend(function(context) {
                        ...
                        $$watchDelegate: function(scope, listener, objectEquality) {
                            ...
                            return scope.$watchGroup(parseFns, function(values, oldValues) {
                                ...
                                if (isFunction(listener)) listener.call(this, currValue, values !== oldValues ? lastValue : currValue, scope);
                                lastValue = currValue;
                            }, objectEquality);
                        }
                    });
        ...
        this.$get = [ "$rootScope", "$window", "$q", "$$q", function($rootScope, $window, $q, $$q) {
            function interval(fn, delay, count, invokeApply) {
                ...
                return count = isDefined(count) ? count : 0, promise.then(null, null, fn), promise.$$intervalId = setInterval(function() {
                    if (deferred.notify(iteration++), count > 0 && iteration >= count) deferred.resolve(iteration), 
                    clearInterval(promise.$$intervalId), delete intervals[promise.$$intervalId];
                    if (!skipApply) $rootScope.$apply();
                }, delay), intervals[promise.$$intervalId] = deferred, promise;
            }
            var intervals = {};
            return interval.cancel = function(promise) {
                if (promise && promise.$$intervalId in intervals) return intervals[promise.$$intervalId].reject("canceled"), 
        ...
        this.$get = function() {
            return {
                ...
                pluralCat: function(num) {
                    if (1 === num) return "one"; else return "other";
                }
            };
        };
    }
    ...
    function parseAppUrl(relativeUrl, locationObj) {
        ...
        if (prefixed) relativeUrl = "/" + relativeUrl;
        var match = urlResolve(relativeUrl);
        if (locationObj.$$path = decodeURIComponent(prefixed && "/" === match.pathname.charAt(0) ? match.pathname.substring(1) : match.pathname), 
        locationObj.$$search = parseKeyValue(match.search), locationObj.$$hash = decodeURIComponent(match.hash), 
        locationObj.$$path && "/" != locationObj.$$path.charAt(0)) locationObj.$$path = "/" + locationObj.$$path;
    }
    function beginsWith(begin, whole) {
        if (0 === whole.indexOf(begin)) return whole.substr(begin.length); else return void 0;
    }
    ...
    function LocationHtml5Url(appBase, basePrefix) {
        ...
        parseAbsoluteUrl(appBase, this), this.$$parse = function(url) {
            ...
            if (parseAppUrl(pathUrl, this), !this.$$path) this.$$path = "/";
            this.$$compose();
            ...
            if ((appUrl = beginsWith(appBase, url)) !== undefined) if (prevAppUrl = appUrl, 
            (appUrl = beginsWith(basePrefix, appUrl)) !== undefined) rewrittenUrl = appBaseNoFile + (beginsWith("/", appUrl) || appUrl); else rewrittenUrl = appBase + prevAppUrl; else if ((appUrl = beginsWith(appBaseNoFile, url)) !== undefined) rewrittenUrl = appBaseNoFile + appUrl; else if (appBaseNoFile == url + "/") rewrittenUrl = appBaseNoFile;
    ...
    function LocationHashbangUrl(appBase, hashPrefix) {
        ...
        parseAbsoluteUrl(appBase, this), this.$$parse = function(url) {
            function removeWindowsDriveName(path, url, base) {
                ...
                if (0 === url.indexOf(base)) url = url.replace(base, "");
                if (windowsFilePathExp.exec(url)) return path; else return firstPathSegmentMatch = windowsFilePathExp.exec(path), 
            ...
            if ("#" === withoutBaseUrl.charAt(0)) {
                if (withoutHashUrl = beginsWith(hashPrefix, withoutBaseUrl), isUndefined(withoutHashUrl)) withoutHashUrl = withoutBaseUrl;
            } else withoutHashUrl = this.$$html5 ? withoutBaseUrl : "";
            ...
            if (stripHash(appBase) == stripHash(url)) return this.$$parse(url), !0; else return !1;
        };
    }
    function LocationHashbangInHtml5Url(appBase, hashPrefix) {
        ...
        this.$$parseLinkUrl = function(url, relHref) {
            ...
            if (appBase == stripHash(url)) rewrittenUrl = url; else if (appUrl = beginsWith(appBaseNoFile, url)) rewrittenUrl = appBase + hashPrefix + appUrl; else if (appBaseNoFile === url + "/") rewrittenUrl = appBaseNoFile;
            if (rewrittenUrl) this.$$parse(rewrittenUrl);
            return !!rewrittenUrl;
        }, this.$$compose = function() {
            var search = toKeyValue(this.$$search), hash = this.$$hash ? "#" + encodeUriSegment(this.$$hash) : "";
    ...
    function locationGetterSetter(property, preprocess) {
        return function(value) {
            if (isUndefined(value)) return this[property]; else return this[property] = preprocess(value), 
            this.$$compose(), this;
        };
    }
    function $LocationProvider() {
        ...
        this.hashPrefix = function(prefix) {
            if (isDefined(prefix)) return hashPrefix = prefix, this; else return hashPrefix;
        }, this.html5Mode = function(mode) {
            if (isBoolean(mode)) return html5Mode.enabled = mode, this; else if (isObject(mode)) {
                if (isBoolean(mode.enabled)) html5Mode.enabled = mode.enabled;
                if (isBoolean(mode.requireBase)) html5Mode.requireBase = mode.requireBase;
                if (isBoolean(mode.rewriteLinks)) html5Mode.rewriteLinks = mode.rewriteLinks;
                return this;
            } else return html5Mode;
            ...
            if ($rootElement.on("click", function(event) {
                if (html5Mode.rewriteLinks && !event.ctrlKey && !event.metaKey && !event.shiftKey && 2 != event.which && 2 != event.button) {
                    ...
                    if (!IGNORE_URI_REGEXP.test(absHref)) if (absHref && !elm.attr("target") && !event.isDefaultPrevented()) if ($location.$$parseLinkUrl(absHref, relHref)) if (event.preventDefault(), 
            ...
            return $browser.onUrlChange(function(newUrl, newState) {
                if ($rootScope.$evalAsync(function() {
                    var defaultPrevented, oldUrl = $location.absUrl(), oldState = $location.$$state;
                    if ($location.$$parse(newUrl), $location.$$state = newState, defaultPrevented = $rootScope.$broadcast("$locationChangeStart", newUrl, oldUrl, newState, oldState).defaultPrevented, 
                    $location.absUrl() === newUrl) if (defaultPrevented) $location.$$parse(oldUrl), 
                    $location.$$state = oldState, setBrowserUrlWithFallback(oldUrl, !1, oldState); else initializing = !1, 
                ...
                if (initializing || urlOrStateChanged) initializing = !1, $rootScope.$evalAsync(function() {
                    ...
                    if ($location.absUrl() === newUrl) if (defaultPrevented) $location.$$parse(oldUrl), 
                    $location.$$state = oldState; else {
                        if (urlOrStateChanged) setBrowserUrlWithFallback(newUrl, currentReplace, oldState === $location.$$state ? null : $location.$$state);
                        afterLocationChange(oldUrl, oldState);
                    }
                });
                $location.$$replace = !1;
    ...
    function $LogProvider() {
        ...
        this.debugEnabled = function(flag) {
            if (isDefined(flag)) return debug = flag, this; else return debug;
        }, this.$get = [ "$window", function($window) {
            function formatError(arg) {
                if (arg instanceof Error) if (arg.stack) arg = arg.message && -1 === arg.stack.indexOf(arg.message) ? "Error: " + arg.message + "\n" + arg.stack : arg.stack; else if (arg.sourceURL) arg = arg.message + "\n" + arg.sourceURL + ":" + arg.line;
                return arg;
            }
            function consoleLog(type) {
                ...
                }; else return function(arg1, arg2) {
            ...
            return {
                ...
                debug: function() {
                    ...
                    return function() {
                        if (debug) fn.apply(self, arguments);
                    };
                }()
            };
        } ];
    ...
    function ensureSafeObject(obj, fullExpression) {
        if (obj) if (obj.constructor === obj) throw $parseMinErr("isecfn", "Referencing Function in Angular expressions is disallowed! Expression: {0}", fullExpression); else if (obj.window === obj) throw $parseMinErr("isecwindow", "Referencing the Window in Angular expressions is disallowed! Expression: {0}", fullExpression); else if (obj.children && (obj.nodeName || obj.prop && obj.attr && obj.find)) throw $parseMinErr("isecdom", "Referencing DOM nodes in Angular expressions is disallowed! Expression: {0}", fullExpression); else if (obj === Object) throw $parseMinErr("isecobj", "Referencing Object in Angular expressions is disallowed! Expression: {0}", fullExpression);
        return obj;
    }
    function ensureSafeFunction(obj, fullExpression) {
        if (obj) if (obj.constructor === obj) throw $parseMinErr("isecfn", "Referencing Function in Angular expressions is disallowed! Expression: {0}", fullExpression); else if (obj === CALL || obj === APPLY || obj === BIND) throw $parseMinErr("isecff", "Referencing call, apply or bind in Angular expressions is disallowed! Expression: {0}", fullExpression);
    }
    ...
    function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, expensiveChecks) {
        ...
        return function(scope, locals) {
            ...
            if (pathVal = eso0(pathVal[key0]), !key1) return pathVal;
            if (null == pathVal) return undefined;
            if (pathVal = eso1(pathVal[key1]), !key2) return pathVal;
            if (null == pathVal) return undefined;
            if (pathVal = eso2(pathVal[key2]), !key3) return pathVal;
            if (null == pathVal) return undefined;
            if (pathVal = eso3(pathVal[key3]), !key4) return pathVal;
            if (null == pathVal) return undefined; else return pathVal = eso4(pathVal[key4]);
        };
    }
    ...
    function getterFn(path, options, fullExp) {
        ...
        if (options.csp) if (6 > pathKeysLength) fn = cspSafeGetterFn(pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4], fullExp, expensiveChecks); else fn = function(scope, locals) {
            var val, i = 0;
            ...
            forEach(pathKeys, function(key, index) {
                ...
                if (expensiveChecks || isPossiblyDangerousMemberName(key)) lookupJs = "eso(" + lookupJs + ", fe)", 
                needsEnsureSafeObject = !0;
                code += "if(s == null) return undefined;\ns=" + lookupJs + ";\n";
            }), code += "return s;";
            var evaledFnGetter = new Function("s", "l", "eso", "fe", code);
            if (evaledFnGetter.toString = valueFn(code), needsEnsureSafeObject) evaledFnGetter = getterFnWithEnsureSafeObject(evaledFnGetter, fullExp);
            fn = evaledFnGetter;
        }
    ...
    function $ParseProvider() {
        ...
        this.$get = [ "$filter", "$sniffer", function($filter, $sniffer) {
            function wrapSharedExpression(exp) {
                var wrapped = exp;
                if (exp.sharedGetter) wrapped = function(self, locals) {
            ...
            function collectExpressionInputs(inputs, list) {
                for (var i = 0, ii = inputs.length; ii > i; i++) {
                    var input = inputs[i];
                    if (!input.constant) if (input.inputs) collectExpressionInputs(input.inputs, list); else if (-1 === list.indexOf(input)) list.push(input);
                }
                return list;
            }
            function expressionInputDirtyCheck(newValue, oldValueOfValue) {
                if (null == newValue || null == oldValueOfValue) return newValue === oldValueOfValue;
                if ("object" == typeof newValue) if (newValue = getValueOf(newValue), "object" == typeof newValue) return !1;
            ...
            function inputsWatchDelegate(scope, listener, objectEquality, parsedExpression) {
                ...
                if (1 === inputExpressions.length) {
                    ...
                    return inputExpressions = inputExpressions[0], scope.$watch(function(scope) {
                        ...
                        if (!expressionInputDirtyCheck(newInputValue, oldInputValue)) lastResult = parsedExpression(scope), 
                ...
                return scope.$watch(function(scope) {
                    for (var changed = !1, i = 0, ii = inputExpressions.length; ii > i; i++) {
                        ...
                        if (changed || (changed = !expressionInputDirtyCheck(newInputValue, oldInputValueOfValues[i]))) oldInputValueOfValues[i] = newInputValue && getValueOf(newInputValue);
                    }
            ...
            function oneTimeWatchDelegate(scope, listener, objectEquality, parsedExpression) {
                ...
                return unwatch = scope.$watch(function(scope) {
                    ...
                    if (lastValue = value, isFunction(listener)) listener.apply(this, arguments);
                    if (isDefined(value)) scope.$$postDigest(function() {
                        if (isDefined(lastValue)) unwatch();
                    });
                }, objectEquality);
            }
            function oneTimeLiteralWatchDelegate(scope, listener, objectEquality, parsedExpression) {
                ...
                return unwatch = scope.$watch(function(scope) {
                    ...
                    if (lastValue = value, isFunction(listener)) listener.call(this, value, old, scope);
                    if (isAllDefined(value)) scope.$$postDigest(function() {
            ...
            function constantWatchDelegate(scope, listener, objectEquality, parsedExpression) {
                ...
                return unwatch = scope.$watch(function(scope) {
                    ...
                    if (isFunction(listener)) listener.apply(this, arguments);
                    unwatch();
                }, objectEquality);
            }
            function addInterceptor(parsedExpression, interceptorFn) {
                ...
                if (parsedExpression.$$watchDelegate && parsedExpression.$$watchDelegate !== inputsWatchDelegate) fn.$$watchDelegate = parsedExpression.$$watchDelegate; else if (!interceptorFn.$stateful) fn.$$watchDelegate = inputsWatchDelegate, 
                ...
                switch (typeof exp) {
                  case "string":
                    ...
                    if (parsedExpression = cache[cacheKey], !parsedExpression) {
                        if (":" === exp.charAt(0) && ":" === exp.charAt(1)) oneTime = !0, exp = exp.substring(2);
                        ...
                        if (parsedExpression = parser.parse(exp), parsedExpression.constant) parsedExpression.$$watchDelegate = constantWatchDelegate; else if (oneTime) parsedExpression = wrapSharedExpression(parsedExpression), 
                        parsedExpression.$$watchDelegate = parsedExpression.literal ? oneTimeLiteralWatchDelegate : oneTimeWatchDelegate; else if (parsedExpression.inputs) parsedExpression.$$watchDelegate = inputsWatchDelegate;
    ...
    function qFactory(nextTick, exceptionHandler) {
        function callOnce(self, resolveFn, rejectFn) {
            function wrap(fn) {
                return function(value) {
                    if (!called) called = !0, fn.call(self, value);
                };
            }
            var called = !1;
        ...
        function processQueue(state) {
            ...
            for (var i = 0, ii = pending.length; ii > i; ++i) {
                ...
                try {
                    if (isFunction(fn)) promise.resolve(fn(state.value)); else if (1 === state.status) promise.resolve(state.value); else promise.reject(state.value);
                } catch (e) {
        ...
        function scheduleProcessQueue(state) {
            if (!state.processScheduled && state.pending) state.processScheduled = !0, nextTick(function() {
                processQueue(state);
            });
        }
        function Deferred() {
        ...
        function all(promises) {
            ...
            if (forEach(promises, function(promise, key) {
                counter++, when(promise).then(function(value) {
                    if (!results.hasOwnProperty(key)) if (results[key] = value, !--counter) deferred.resolve(results);
                }, function(reason) {
                    if (!results.hasOwnProperty(key)) deferred.reject(reason);
                });
        ...
        Promise.prototype = {
            then: function(onFulfilled, onRejected, progressBack) {
                ...
                if (this.$$state.pending = this.$$state.pending || [], this.$$state.pending.push([ result, onFulfilled, onRejected, progressBack ]), 
            ...
            resolve: function(val) {
                if (!this.promise.$$state.status) if (val === this.promise) this.$$reject($qMinErr("qcycle", "Expected promise to be resolved with value other than itself '{0}'", val)); else this.$$resolve(val);
            },
            $$resolve: function(val) {
                var then, fns;
                ...
                try {
                    ...
                    if (isFunction(then)) this.promise.$$state.status = -1, then.call(val, fns[0], fns[1], this.notify); else this.promise.$$state.value = val, 
            ...
            reject: function(reason) {
                if (!this.promise.$$state.status) this.$$reject(reason);
            },
            $$reject: function(reason) {
            ...
            notify: function(progress) {
                ...
                if (this.promise.$$state.status <= 0 && callbacks && callbacks.length) nextTick(function() {
        ...
        var reject = function(reason) {
            ...
            var result = new Deferred();
            if (resolved) result.resolve(value); else result.reject(value);
            return result.promise;
            ...
            if (isPromiseLike(callbackOutput)) return callbackOutput.then(function() {
                return makePromise(value, isResolved);
            }, function(error) {
                return makePromise(error, !1);
            }); else return makePromise(value, isResolved);
    ...
    function $RootScopeProvider() {
        ...
        this.digestTtl = function(value) {
            ...
            function decrementListenerCount(current, count, name) {
                do if (current.$$listenerCount[name] -= count, 0 === current.$$listenerCount[name]) delete current.$$listenerCount[name]; while (current = current.$parent);
            }
            ...
            function scheduleApplyAsync() {
                if (null === applyAsyncId) applyAsyncId = $browser.defer(function() {
            ...
            Scope.prototype = {
                ...
                $new: function(isolate, parent) {
                    ...
                    var child;
                    if (parent = parent || this, isolate) child = new Scope(), child.$root = this.$root; else {
                        if (!this.$$ChildScope) this.$$ChildScope = function() {
                            this.$$watchers = this.$$nextSibling = this.$$childHead = this.$$childTail = null, 
                            this.$$listeners = {}, this.$$listenerCount = {}, this.$id = nextUid(), this.$$ChildScope = null;
                        }, this.$$ChildScope.prototype = this;
                        child = new this.$$ChildScope();
                    }
                    if (child.$parent = parent, child.$$prevSibling = parent.$$childTail, parent.$$childHead) parent.$$childTail.$$nextSibling = child, 
                    parent.$$childTail = child; else parent.$$childHead = parent.$$childTail = child;
                ...
                $watch: function(watchExp, listener, objectEquality) {
                    ...
                    if (lastDirtyWatch = null, !isFunction(listener)) watcher.fn = noop;
                    if (!array) array = scope.$$watchers = [];
                    return array.unshift(watcher), function() {
                        arrayRemove(array, watcher), lastDirtyWatch = null;
                    };
                },
                $watchGroup: function(watchExpressions, listener) {
                    function watchGroupAction() {
                        if (changeReactionScheduled = !1, firstRun) firstRun = !1, listener(newValues, newValues, self); else listener(newValues, oldValues, self);
                    }
                    ...
                    }); else return forEach(watchExpressions, function(expr, i) {
                        var unwatchFn = self.$watch(expr, function(value, oldValue) {
                            if (newValues[i] = value, oldValues[i] = oldValue, !changeReactionScheduled) changeReactionScheduled = !0, 
                ...
                $watchCollection: function(obj, listener) {
                    function $watchCollectionInterceptor(_value) {
                        ...
                        if (!isUndefined(newValue)) {
                            if (!isObject(newValue)) {
                                if (oldValue !== newValue) oldValue = newValue, changeDetected++;
                            } else if (isArrayLike(newValue)) {
                                if (oldValue !== internalArray) oldValue = internalArray, oldLength = oldValue.length = 0, 
                                changeDetected++;
                                if (newLength = newValue.length, oldLength !== newLength) changeDetected++, oldValue.length = oldLength = newLength;
                                for (var i = 0; newLength > i; i++) if (oldItem = oldValue[i], newItem = newValue[i], 
                                bothNaN = oldItem !== oldItem && newItem !== newItem, !bothNaN && oldItem !== newItem) changeDetected++, 
                                oldValue[i] = newItem;
                            } else {
                                if (oldValue !== internalObject) oldValue = internalObject = {}, oldLength = 0, 
                                changeDetected++;
                                newLength = 0;
                                for (key in newValue) if (newValue.hasOwnProperty(key)) if (newLength++, newItem = newValue[key], 
                                oldItem = oldValue[key], key in oldValue) {
                                    if (bothNaN = oldItem !== oldItem && newItem !== newItem, !bothNaN && oldItem !== newItem) changeDetected++, 
                                    oldValue[key] = newItem;
                                } else oldLength++, oldValue[key] = newItem, changeDetected++;
                                if (oldLength > newLength) {
                                    changeDetected++;
                                    for (key in oldValue) if (!newValue.hasOwnProperty(key)) oldLength--, delete oldValue[key];
                                }
                            }
                    ...
                    function $watchCollectionAction() {
                        if (initRun) initRun = !1, listener(newValue, newValue, self); else listener(newValue, veryOldValue, self);
                        if (trackVeryOldValue) if (!isObject(newValue)) veryOldValue = newValue; else if (isArrayLike(newValue)) {
                            veryOldValue = new Array(newValue.length);
                            for (var i = 0; i < newValue.length; i++) veryOldValue[i] = newValue[i];
                        } else {
                            veryOldValue = {};
                            for (var key in newValue) if (hasOwnProperty.call(newValue, key)) veryOldValue[key] = newValue[key];
                        }
                    }
                ...
                $digest: function() {
                    ...
                    if (beginPhase("$digest"), $browser.$$checkUrlChange(), this === $rootScope && null !== applyAsyncId) $browser.defer.cancel(applyAsyncId), 
                    flushApplyAsync();
                    lastDirtyWatch = null;
                    do {
                        ...
                        traverseScopesLoop: do {
                            if (watchers = current.$$watchers) for (length = watchers.length; length--; ) try {
                                if (watch = watchers[length]) if ((value = watch.get(current)) !== (last = watch.last) && !(watch.eq ? equals(value, last) : "number" == typeof value && "number" == typeof last && isNaN(value) && isNaN(last))) {
                                    if (dirty = !0, lastDirtyWatch = watch, watch.last = watch.eq ? copy(value, null) : value, 
                                    watch.fn(value, last === initWatchVal ? value : last, current), 5 > ttl) {
                                        if (logIdx = 4 - ttl, !watchLog[logIdx]) watchLog[logIdx] = [];
                ...
                $evalAsync: function(expr, locals) {
                    if (!$rootScope.$$phase && !asyncQueue.length) $browser.defer(function() {
                        if (asyncQueue.length) $rootScope.$digest();
                    });
                    asyncQueue.push({
                        scope: this,
                        expression: expr,
                ...
                $on: function(name, listener) {
                    ...
                    if (!namedListeners) this.$$listeners[name] = namedListeners = [];
                    namedListeners.push(listener);
                    var current = this;
                    do {
                        if (!current.$$listenerCount[name]) current.$$listenerCount[name] = 0;
                        current.$$listenerCount[name]++;
                    } while (current = current.$parent);
                    var self = this;
                    return function() {
                        var indexOfListener = namedListeners.indexOf(listener);
                        if (-1 !== indexOfListener) namedListeners[indexOfListener] = null, decrementListenerCount(self, 1, name);
                    };
                },
                $emit: function(name) {
    ...
    function $$SanitizeUriProvider() {
        ...
        this.aHrefSanitizationWhitelist = function(regexp) {
            if (isDefined(regexp)) return aHrefSanitizationWhitelist = regexp, this; else return aHrefSanitizationWhitelist;
        }, this.imgSrcSanitizationWhitelist = function(regexp) {
            if (isDefined(regexp)) return imgSrcSanitizationWhitelist = regexp, this; else return imgSrcSanitizationWhitelist;
        }, this.$get = function() {
            return function(uri, isImage) {
                var normalizedVal, regex = isImage ? imgSrcSanitizationWhitelist : aHrefSanitizationWhitelist;
                if (normalizedVal = urlResolve(uri).href, "" !== normalizedVal && !normalizedVal.match(regex)) return "unsafe:" + normalizedVal; else return uri;
            };
        };
    }
    function adjustMatcher(matcher) {
        if ("self" === matcher) return matcher; else if (isString(matcher)) {
            if (matcher.indexOf("***") > -1) throw $sceMinErr("iwcard", "Illegal sequence *** in string matcher.  String: {0}", matcher);
            return matcher = escapeForRegexp(matcher).replace("\\*\\*", ".*").replace("\\*", "[^:/.?&;]*"), 
            new RegExp("^" + matcher + "$");
        } else if (isRegExp(matcher)) return new RegExp("^" + matcher.source + "$"); else throw $sceMinErr("imatcher", 'Matchers may only be "self", string patterns or RegExp objects');
    }
    function adjustMatchers(matchers) {
        var adjustedMatchers = [];
        if (isDefined(matchers)) forEach(matchers, function(matcher) {
    ...
    function $SceDelegateProvider() {
        ...
        this.resourceUrlWhitelist = function(value) {
            if (arguments.length) resourceUrlWhitelist = adjustMatchers(value);
            return resourceUrlWhitelist;
        }, this.resourceUrlBlacklist = function(value) {
            if (arguments.length) resourceUrlBlacklist = adjustMatchers(value);
            return resourceUrlBlacklist;
        }, this.$get = [ "$injector", function($injector) {
            function matchUrl(matcher, parsedUrl) {
                if ("self" === matcher) return urlIsSameOrigin(parsedUrl); else return !!matcher.exec(parsedUrl.href);
            }
            ...
            function generateHolderType(Base) {
                ...
                if (Base) holderType.prototype = new Base();
                return holderType.prototype.valueOf = function() {
            ...
            function valueOf(maybeTrusted) {
                if (maybeTrusted instanceof trustedValueHolderBase) return maybeTrusted.$$unwrapTrustedValue(); else return maybeTrusted;
            }
            ...
            if ($injector.has("$sanitize")) htmlSanitizer = $injector.get("$sanitize");
    ...
    function $SceProvider() {
        ...
        this.enabled = function(value) {
            if (arguments.length) enabled = !!value;
            return enabled;
            ...
            sce.parseAs = function(type, expr) {
                ...
                if (parsed.literal && parsed.constant) return parsed; else return $parse(expr, function(value) {
    ...
    function $SnifferProvider() {
        this.$get = [ "$window", "$document", function($window, $document) {
            ...
            if (bodyStyle) {
                ...
                if (transitions = !!("transition" in bodyStyle || vendorPrefix + "Transition" in bodyStyle), 
    ...
    function $TemplateRequestProvider() {
        this.$get = [ "$templateCache", "$http", "$q", function($templateCache, $http, $q) {
            function handleRequestFn(tpl, ignoreRequestError) {
                ...
                if (isArray(transformResponse)) transformResponse = transformResponse.filter(function(transformer) {
                    return transformer !== defaultHttpResponseTransform;
                }); else if (transformResponse === defaultHttpResponseTransform) transformResponse = null;
                var httpOptions = {
                    cache: $templateCache,
        ...
        this.$get = [ "$rootScope", "$browser", "$location", function($rootScope, $browser, $location) {
            ...
            return testability.findBindings = function(element, expression, opt_exactMatch) {
                ...
                return forEach(bindings, function(binding) {
                    ...
                    if (dataBinding) forEach(dataBinding, function(bindingName) {
                        if (opt_exactMatch) {
                            var matcher = new RegExp("(^|\\s)" + escapeForRegexp(expression) + "(\\s|\\||$)");
                            if (matcher.test(bindingName)) matches.push(binding);
                        } else if (-1 != bindingName.indexOf(expression)) matches.push(binding);
                    });
                }), matches;
                ...
                if (url !== $location.url()) $location.url(url), $rootScope.$digest();
    ...
    function $TimeoutProvider() {
        this.$get = [ "$rootScope", "$browser", "$q", "$$q", "$exceptionHandler", function($rootScope, $browser, $q, $$q, $exceptionHandler) {
            ...
            return timeout.cancel = function(promise) {
                if (promise && promise.$$timeoutId in deferreds) return deferreds[promise.$$timeoutId].reject("canceled"), 
                delete deferreds[promise.$$timeoutId], $browser.defer.cancel(promise.$$timeoutId); else return !1;
            }, timeout;
        } ];
    }
    function urlResolve(url) {
        ...
        return urlParsingNode.setAttribute("href", href), {
    ...
    function $FilterProvider($provide) {
        function register(name, factory) {
            ...
            } else return $provide.factory(name + suffix, factory);
        }
        var suffix = "Filter";
    ...
    function createPredicateFn(expression, comparator, matchAgainstAnyProp) {
        ...
        if (comparator === !0) comparator = equals; else if (!isFunction(comparator)) comparator = function(actual, expected) {
            if (isObject(actual) || isObject(expected)) return !1; else return actual = lowercase("" + actual), 
            expected = lowercase("" + expected), -1 !== actual.indexOf(expected);
        };
        return predicateFn = function(item) {
            if (shouldMatchPrimitives && !isObject(item)) return deepCompare(item, expression.$, comparator, !1); else return deepCompare(item, expression, comparator, matchAgainstAnyProp);
        };
    }
    function deepCompare(actual, expected, comparator, matchAgainstAnyProp, dontMatchWholeObject) {
        var actualType = typeof actual, expectedType = typeof expected;
        if ("string" === expectedType && "!" === expected.charAt(0)) return !deepCompare(actual, expected.substring(1), comparator, matchAgainstAnyProp); else if (isArray(actual)) return actual.some(function(item) {
        ...
        switch (actualType) {
          case "object":
            ...
            if (matchAgainstAnyProp) {
                ...
                return !0;
            } else return comparator(actual, expected);
    ...
    function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
        ...
        if (-1 !== numStr.indexOf("e")) {
            ...
            if (match && "-" == match[2] && match[3] > fractionSize + 1) number = 0; else formatedText = numStr, 
            hasExponent = !0;
        }
        if (!hasExponent) {
            ...
            if (whole.length >= lgroup + group) for (pos = whole.length - lgroup, i = 0; pos > i; i++) {
                if ((pos - i) % group === 0 && 0 !== i) formatedText += groupSep;
                formatedText += whole.charAt(i);
            }
            for (i = pos; i < whole.length; i++) {
                if ((whole.length - i) % lgroup === 0 && 0 !== i) formatedText += groupSep;
                formatedText += whole.charAt(i);
            }
            for (;fraction.length < fractionSize; ) fraction += "0";
            if (fractionSize && "0" !== fractionSize) formatedText += decimalSep + fraction.substr(0, fractionSize);
        } else if (fractionSize > 0 && 1 > number) formatedText = number.toFixed(fractionSize), 
        number = parseFloat(formatedText);
        if (0 === number) isNegative = !1;
        return parts.push(isNegative ? pattern.negPre : pattern.posPre, formatedText, isNegative ? pattern.negSuf : pattern.posSuf), 
        parts.join("");
    }
    function padNumber(num, digits, trim) {
        var neg = "";
        if (0 > num) neg = "-", num = -num;
        for (num = "" + num; num.length < digits; ) num = "0" + num;
    ...
    function dateFilter($locale) {
        function jsonStringToDate(string) {
            ...
            if (match = string.match(R_ISO8601_STR)) {
                ...
                if (match[9]) tzHour = int(match[9] + match[10]), tzMin = int(match[9] + match[11]);
        ...
        return function(date, format, timezone) {
            ...
            for (;format; ) if (match = DATE_FORMATS_SPLIT.exec(format)) parts = concat(parts, match, 1), 
            format = parts.pop(); else parts.push(format), format = null;
            if (timezone && "UTC" === timezone) date = new Date(date.getTime()), date.setMinutes(date.getMinutes() + date.getTimezoneOffset());
    ...
    function limitToFilter() {
        return function(input, limit) {
            ...
            if (1/0 === Math.abs(Number(limit))) limit = Number(limit); else limit = int(limit);
    ...
    function orderByFilter($parse) {
        return function(array, sortPredicate, reverseOrder) {
            ...
            function objectToString(value) {
                ...
                if ("function" == typeof value.valueOf) if (value = value.valueOf(), isPrimitive(value)) return value;
                if ("function" == typeof value.toString) if (value = value.toString(), isPrimitive(value)) return value;
                return "";
            }
            function compare(v1, v2) {
                var t1 = typeof v1, t2 = typeof v2;
                if (t1 === t2 && "object" === t1) v1 = objectToString(v1), v2 = objectToString(v2);
                if (t1 === t2) {
                    if ("string" === t1) v1 = v1.toLowerCase(), v2 = v2.toLowerCase();
                    if (v1 === v2) return 0; else return v2 > v1 ? -1 : 1;
                } else return t2 > t1 ? -1 : 1;
            }
            if (!isArrayLike(array)) return array;
            if (sortPredicate = isArray(sortPredicate) ? sortPredicate : [ sortPredicate ], 
            0 === sortPredicate.length) sortPredicate = [ "+" ];
            return sortPredicate = sortPredicate.map(function(predicate) {
    ...
    function ngDirective(directive) {
        ...
        return directive.restrict = directive.restrict || "AC", valueFn(directive);
    }
    ...
    function FormController(element, attrs, $scope, $animate, $interpolate) {
        ...
        parentForm.$addControl(form), form.$rollbackViewValue = function() {
            ...
            if (assertNotHasOwnProperty(control.$name, "input"), controls.push(control), control.$name) form[control.$name] = control;
        }, form.$$renameControl = function(control, newName) {
            var oldName = control.$name;
            if (form[oldName] === control) delete form[oldName];
            form[newName] = control, control.$name = newName;
        }, form.$removeControl = function(control) {
            if (control.$name && form[control.$name] === control) delete form[control.$name];
            ...
            set: function(object, property, controller) {
                ...
                if (!list) object[property] = [ controller ]; else {
                    var index = list.indexOf(controller);
                    if (-1 === index) list.push(controller);
                }
            },
            unset: function(object, property, controller) {
                var list = object[property];
                if (list) if (arrayRemove(list, controller), 0 === list.length) delete object[property];
            },
    ...
    function baseInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        var listener = function(ev) {
            if (timeout) $browser.defer.cancel(timeout), timeout = null;
            if (!composing) {
                var value = element.val(), event = ev && ev.type;
                if ("password" !== type && (!attr.ngTrim || "false" !== attr.ngTrim)) value = trim(value);
                if (ctrl.$viewValue !== value || "" === value && ctrl.$$hasNativeValidators) ctrl.$setViewValue(value, event);
            }
        };
        if ($sniffer.hasEvent("input")) element.on("input", listener); else {
            var timeout, deferListener = function(ev, input, origValue) {
                if (!timeout) timeout = $browser.defer(function() {
                    if (timeout = null, !input || input.value !== origValue) listener(ev);
                });
            };
            if (element.on("keydown", function(event) {
                var key = event.keyCode;
                if (!(91 === key || key > 15 && 19 > key || key >= 37 && 40 >= key)) deferListener(event, this, this.value);
    ...
    function weekParser(isoWeek, existingDate) {
        ...
        if (isString(isoWeek)) {
            ...
            if (parts) {
                ...
                if (existingDate) hours = existingDate.getHours(), minutes = existingDate.getMinutes(), 
        ...
        return function(iso, date) {
            ...
            if (isString(iso)) {
                ...
                if (regexp.lastIndex = 0, parts = regexp.exec(iso)) {
                    ...
                    return forEach(parts, function(part, index) {
                        if (index < mapping.length) map[mapping[index]] = +part;
        ...
        return function(scope, element, attr, ctrl, $sniffer, $browser, $filter) {
            ...
            if (ctrl.$$parserName = type, ctrl.$parsers.push(function(value) {
                ...
                if (regexp.test(value)) {
                    ...
                    if ("UTC" === timezone) parsedDate.setMinutes(parsedDate.getMinutes() - parsedDate.getTimezoneOffset());
                    return parsedDate;
                }
                return undefined;
            }), ctrl.$formatters.push(function(value) {
    ...
    function badInputChecker(scope, element, attr, ctrl) {
        ...
        if (nativeValidation) ctrl.$parsers.push(function(value) {
    ...
    function numberInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        ctrl.$$parserName = "number", ctrl.$parsers.push(function(value) {
            if (ctrl.$isEmpty(value)) return null;
            if (NUMBER_REGEXP.test(value)) return parseFloat(value); else return undefined;
        }), ctrl.$formatters.push(function(value) {
            ...
            ctrl.$validators.min = function(value) {
                ...
                if (isDefined(val) && !isNumber(val)) val = parseFloat(val, 10);
    ...
    function radioInputType(scope, element, attr, ctrl) {
        if (isUndefined(attr.name)) element.attr("name", nextUid());
        var listener = function(ev) {
            if (element[0].checked) ctrl.$setViewValue(attr.value, ev && ev.type);
        };
    ...
    function classDirective(name, selector) {
        return name = "ngClass" + name, [ "$animate", function($animate) {
            ...
            function arrayClasses(classVal) {
                if (isArray(classVal)) return classVal; else if (isString(classVal)) return classVal.split(" "); else if (isObject(classVal)) {
                    var classes = [];
                    return forEach(classVal, function(v, k) {
                        if (v) classes = classes.concat(k.split(" "));
                    }), classes;
                }
                return classVal;
            }
                ...
                link: function(scope, element, attr) {
                    ...
                    function digestClassCounts(classes, count) {
                        ...
                        return forEach(classes, function(className) {
                            if (count > 0 || classCounts[className]) if (classCounts[className] = (classCounts[className] || 0) + count, 
                    ...
                    function updateClasses(oldClasses, newClasses) {
                        ...
                        if (toAdd = digestClassCounts(toAdd, 1), toRemove = digestClassCounts(toRemove, -1), 
                    ...
                    function ngClassWatchAction(newVal) {
                        if (selector === !0 || scope.$index % 2 === selector) {
                            ...
                            if (!oldVal) addClasses(newClasses); else if (!equals(newVal, oldVal)) {
                    ...
                    var oldVal;
                    if (scope.$watch(attr[name], ngClassWatchAction, !0), attr.$observe("class", function() {
    ...
    function addSetValidityMethod(context) {
        function setValidity(validationErrorKey, state, controller) {
            if (state === undefined) createAndSet("$pending", validationErrorKey, controller); else unsetAndCleanup("$pending", validationErrorKey, controller);
            if (!isBoolean(state)) unset(ctrl.$error, validationErrorKey, controller), unset(ctrl.$$success, validationErrorKey, controller); else if (state) unset(ctrl.$error, validationErrorKey, controller), 
            set(ctrl.$$success, validationErrorKey, controller); else set(ctrl.$error, validationErrorKey, controller), 
            unset(ctrl.$$success, validationErrorKey, controller);
            if (ctrl.$pending) cachedToggleClass(PENDING_CLASS, !0), ctrl.$valid = ctrl.$invalid = undefined, 
            toggleValidationCss("", null); else cachedToggleClass(PENDING_CLASS, !1), ctrl.$valid = isObjectEmpty(ctrl.$error), 
            ctrl.$invalid = !ctrl.$valid, toggleValidationCss("", ctrl.$valid);
            var combinedState;
            if (ctrl.$pending && ctrl.$pending[validationErrorKey]) combinedState = undefined; else if (ctrl.$error[validationErrorKey]) combinedState = !1; else if (ctrl.$$success[validationErrorKey]) combinedState = !0; else combinedState = null;
        ...
        function unsetAndCleanup(name, value, controller) {
            if (ctrl[name]) unset(ctrl[name], value, controller);
            if (isObjectEmpty(ctrl[name])) ctrl[name] = undefined;
        }
        function cachedToggleClass(className, switchValue) {
            if (switchValue && !classCache[className]) $animate.addClass($element, className), 
            classCache[className] = !0; else if (!switchValue && classCache[className]) $animate.removeClass($element, className), 
    ...
    var JQLitePrototype = JQLite.prototype = {
        ready: function(fn) {
            ...
            var fired = !1;
            if ("complete" === document.readyState) setTimeout(trigger); else this.on("DOMContentLoaded", trigger), 
    ...
    forEach({
        ...
        css: function(element, name, value) {
            if (name = camelCase(name), isDefined(value)) element.style[name] = value; else return element.style[name];
        },
        attr: function(element, name, value) {
            var lowercasedName = lowercase(name);
            if (BOOLEAN_ATTR[lowercasedName]) if (isDefined(value)) if (value) element[name] = !0, 
            element.setAttribute(name, lowercasedName); else element[name] = !1, element.removeAttribute(lowercasedName); else return element[name] || (element.attributes.getNamedItem(name) || noop).specified ? lowercasedName : undefined; else if (isDefined(value)) element.setAttribute(name, value); else if (element.getAttribute) {
                var ret = element.getAttribute(name, 2);
                return null === ret ? undefined : ret;
            }
        },
        prop: function(element, name, value) {
            if (isDefined(value)) element[name] = value; else return element[name];
        },
        text: function() {
        ...
        html: function(element, value) {
            if (isUndefined(value)) return element.innerHTML; else return jqLiteDealoc(element, !0), 
        ...
        on: function jqLiteOn(element, type, fn, unsupported) {
            ...
            if (jqLiteAcceptsData(element)) {
                ...
                if (!handle) handle = expandoStore.handle = createEventHandler(element, events);
                for (var types = type.indexOf(" ") >= 0 ? type.split(" ") : [ type ], i = types.length; i--; ) {
                    type = types[i];
                    var eventFns = events[type];
                    if (!eventFns) {
                        if (events[type] = [], "mouseenter" === type || "mouseleave" === type) jqLiteOn(element, MOUSE_EVENT_MAP[type], function(event) {
                            var target = this, related = event.relatedTarget;
                            if (!related || related !== target && !target.contains(related)) handle(event, type);
                        }); else if ("$destroy" !== type) addEventListenerFn(element, type, handle);
        ...
        replaceWith: function(element, replaceNode) {
            ...
            jqLiteDealoc(element), forEach(new JQLite(replaceNode), function(node) {
                if (index) parent.insertBefore(node, index.nextSibling); else parent.replaceChild(node, element);
                index = node;
            });
        },
        ...
        wrap: function(element, wrapNode) {
            ...
            var parent = element.parentNode;
            if (parent) parent.replaceChild(wrapNode, element);
        ...
        toggleClass: function(element, selector, condition) {
            if (selector) forEach(selector.split(" "), function(className) {
                var classCondition = condition;
                if (isUndefined(classCondition)) classCondition = !jqLiteHasClass(element, className);
        ...
        find: function(element, selector) {
            if (element.getElementsByTagName) return element.getElementsByTagName(selector); else return [];
        },
        clone: jqLiteClone,
        triggerHandler: function(element, event, extraParameters) {
            ...
            if (eventFns) {
                ...
                eventFnsCopy = shallowCopy(eventFns), handlerArgs = extraParameters ? [ dummyEvent ].concat(extraParameters) : [ dummyEvent ], 
                forEach(eventFnsCopy, function(fn) {
                    if (!dummyEvent.isImmediatePropagationStopped()) fn.apply(element, handlerArgs);
                });
            }
        }
    }, function(fn, name) {
        JQLite.prototype[name] = function(arg1, arg2, arg3) {
            for (var value, i = 0, ii = this.length; ii > i; i++) if (isUndefined(value)) {
                if (value = fn(this[i], arg1, arg2, arg3), isDefined(value)) value = jqLite(value);
        ...
        this.$$selectors = {}, this.register = function(name, factory) {
            ...
            function resolveElementClasses(element, classes) {
                ...
                return forEach((element.attr("class") || "").split(/\s+/), function(className) {
                    ...
                    if (status === !1 && hasClass) toRemove.push(className); else if (status === !0 && !hasClass) toAdd.push(className);
            ...
            function asyncPromise() {
                if (!currentDefer) currentDefer = $$q.defer(), $$asyncCallback(function() {
            ...
            return {
                ...
                $$addClassImmediately: function(element, className, options) {
                    return element = jqLite(element), className = !isString(className) ? isArray(className) ? className.join(" ") : "" : className, 
                ...
                setClass: function(element, add, remove, options) {
                    ...
                    if (!cache) cache = {
                        classes: {},
                        options: options
                    }, createdCache = !0; else if (options && cache.options) cache.options = angular.extend(cache.options || {}, options);
                    var classes = cache.classes;
                    if (add = isArray(add) ? add : add.split(" "), remove = isArray(remove) ? remove : remove.split(" "), 
                    ...
                    createdCache) cache.promise = runAnimationPostDigest(function(done) {
                        ...
                        if (element.removeData(STORAGE_KEY), cache) {
                            ...
                            if (classes) self.$$setClassImmediately(element, classes[0], classes[1], cache.options);
                        }
                        done();
    ...
    var PREFIX_REGEXP = /^((?:x|data)[\:\-_])/i, $controllerMinErr = minErr("$controller"), APPLICATION_JSON = "application/json", CONTENT_TYPE_APPLICATION_JSON = {
        ...
        search: function(search, paramValue) {
            switch (arguments.length) {
              ...
              case 1:
                if (isString(search) || isNumber(search)) search = search.toString(), this.$$search = parseKeyValue(search); else if (isObject(search)) search = copy(search, {}), 
                forEach(search, function(value, key) {
                    if (null == value) delete search[key];
                }), this.$$search = search; else throw $locationMinErr("isrcharg", "The first argument of the `$location#search()` call must be a string or an object.");
                break;

              default:
                if (isUndefined(paramValue) || null === paramValue) delete this.$$search[search]; else this.$$search[search] = paramValue;
            }
    ...
    var OPERATORS = extend(createMap(), {
        "+": function(self, locals, a, b) {
            if (a = a(self, locals), b = b(self, locals), isDefined(a)) if (isDefined(b)) return a + b; else return a;
        ...
        readNumber: function() {
            for (var number = "", start = this.index; this.index < this.text.length; ) {
                ...
                if ("." == ch || this.isNumber(ch)) number += ch; else {
                    ...
                    if ("e" == ch && this.isExpOperator(peekCh)) number += ch; else if (this.isExpOperator(ch) && peekCh && this.isNumber(peekCh) && "e" == number.charAt(number.length - 1)) number += ch; else if (this.isExpOperator(ch) && (!peekCh || !this.isNumber(peekCh)) && "e" == number.charAt(number.length - 1)) this.throwError("Invalid exponent"); else break;
                }
                this.index++;
            }
            this.tokens.push({
        ...
        readString: function(quote) {
            ...
            for (var string = "", rawString = quote, escape = !1; this.index < this.text.length; ) {
                ...
                if (rawString += ch, escape) {
                    ...
                    escape = !1;
                } else if ("\\" === ch) escape = !0; else if (ch === quote) return this.index++, 
    ...
    Parser.ZERO = extend(function() {
        ...
        parse: function(text) {
            ...
            if (0 !== this.tokens.length) this.throwError("is an unexpected token", this.tokens[0]);
            return value.literal = !!value.literal, value.constant = !!value.constant, value;
        },
        primary: function() {
            var primary;
            if (this.expect("(")) primary = this.filterChain(), this.consume(")"); else if (this.expect("[")) primary = this.arrayDeclaration(); else if (this.expect("{")) primary = this.object(); else if (this.peek().identifier && this.peek().text in CONSTANTS) primary = CONSTANTS[this.consume().text]; else if (this.peek().identifier) primary = this.identifier(); else if (this.peek().constant) primary = this.constant(); else this.throwError("not a primary expression", this.peek());
            for (var next, context; next = this.expect("(", "[", "."); ) if ("(" === next.text) primary = this.functionCall(primary, context), 
            context = null; else if ("[" === next.text) context = primary, primary = this.objectIndex(primary); else if ("." === next.text) context = primary, 
            primary = this.fieldAccess(primary); else this.throwError("IMPOSSIBLE");
            return primary;
        },
        ...
        expect: function(e1, e2, e3, e4) {
            ...
            if (token) return this.tokens.shift(), token; else return !1;
        },
        consume: function(e1) {
            if (0 === this.tokens.length) throw $parseMinErr("ueoe", "Unexpected end of expression: {0}", this.text);
            var token = this.expect(e1);
            if (!token) this.throwError("is unexpected, expecting [" + e1 + "]", this.peek());
            return token;
        },
        ...
        statements: function() {
            for (var statements = []; ;) {
                ...
                if (!this.expect(";")) return 1 === statements.length ? statements[0] : function(self, locals) {
        ...
        assignment: function() {
            ...
            if (token = this.expect("=")) {
                if (!left.assign) this.throwError("implies assignment but [" + this.text.substring(0, token.index) + "] can not be assigned to", token);
                return right = this.ternary(), extend(function(scope, locals) {
        ...
        ternary: function() {
            ...
            if (token = this.expect("?")) if (middle = this.assignment(), this.consume(":")) {
        ...
        unary: function() {
            var token;
            if (this.expect("+")) return this.primary(); else if (token = this.expect("-")) return this.binaryFn(Parser.ZERO, token.text, this.unary()); else if (token = this.expect("!")) return this.unaryFn(token.text, this.unary()); else return this.primary();
        },
        ...
        objectIndex: function(obj) {
            ...
            return this.consume("]"), extend(function(self, locals) {
                ...
                if (ensureSafeMemberName(i, expression), !o) return undefined; else return v = ensureSafeObject(o[i], expression);
            }, {
        ...
        object: function() {
            ...
            if ("}" !== this.peekToken().text) do {
                ...
                if (token.constant) keys.push(token.value); else if (token.identifier) keys.push(token.text); else this.throwError("invalid key", token);
    ...
    var htmlAnchorDirective = valueFn({
        ...
        compile: function(element, attr) {
            if (!attr.href && !attr.xlinkHref && !attr.name) return function(scope, element) {
        ...
        ngAttributeAliasDirectives[normalized] = function() {
            return {
                ...
                link: function(scope, element, attr) {
                    ...
                    if ("href" === attrName && "[object SVGAnimatedString]" === toString.call(element.prop("href"))) name = "xlinkHref", 
                    attr.$attr[name] = "xlink:href", propName = null;
                    attr.$observe(normalized, function(value) {
                        if (value) {
                            if (attr.$set(name, value), msie && propName) element.prop(propName, attr[name]);
                        } else if ("href" === attrName) attr.$set(name, null);
                    });
                }
            };
        };
    ...
    var formDirectiveFactory = function(isNgForm) {
                ...
                compile: function(formElement) {
                    return formElement.addClass(PRISTINE_CLASS).addClass(VALID_CLASS), {
                        pre: function(scope, formElement, attr, controller) {
                            ...
                            if (alias) setter(scope, null, alias, controller, alias), attr.$observe(attr.name ? "name" : "ngForm", function(newValue) {
                                if (alias !== newValue) setter(scope, null, alias, undefined, alias), alias = newValue, 
                                setter(scope, null, alias, controller, alias), parentFormCtrl.$$renameControl(controller, alias);
                            });
                            formElement.on("$destroy", function() {
                                if (parentFormCtrl.$removeControl(controller), alias) setter(scope, null, alias, undefined, alias);
        ...
        return {
            ...
            link: {
                pre: function(scope, element, attr, ctrls) {
                    if (ctrls[0]) (inputType[lowercase(attr.type)] || inputType.text)(scope, element, attr, ctrls[0], $sniffer, $browser, $filter, $parse);
                }
            }
        };
        ...
        return {
            ...
            compile: function(tpl, tplAttr) {
                if (CONSTANT_VALUE_REGEXP.test(tplAttr.ngValue)) return function(scope, elm, attr) {
                    attr.$set("value", scope.$eval(attr.ngValue));
                }; else return function(scope, elm, attr) {
                ...
                compile: function($element, attr) {
                    ...
                    return function(scope, element) {
                        element.on(eventName, function(event) {
                            ...
                            if (forceAsyncEvents[eventName] && $rootScope.$$phase) scope.$evalAsync(callback); else scope.$apply(callback);
                        });
                    };
                }
            };
    ...
    var ngIfDirective = [ "$animate", function($animate) {
        return {
            ...
            link: function($scope, $element, $attr, ctrl, $transclude) {
                ...
                $scope.$watch($attr.ngIf, function(value) {
                    if (value) {
                        if (!childScope) $transclude(function(clone, newScope) {
                        ...
                        if (previousElements) previousElements.remove(), previousElements = null;
                        if (childScope) childScope.$destroy(), childScope = null;
                        if (block) previousElements = getBlockNodes(block.clone), $animate.leave(previousElements).then(function() {
            ...
            compile: function(element, attr) {
                ...
                return function(scope, $element, $attr, ctrl, $transclude) {
                    var currentScope, previousElement, currentElement, changeCounter = 0, cleanupLastIncludeContent = function() {
                        if (previousElement) previousElement.remove(), previousElement = null;
                        if (currentScope) currentScope.$destroy(), currentScope = null;
                        if (currentElement) $animate.leave(currentElement).then(function() {
                    ...
                    scope.$watch($sce.parseAsResourceUrl(srcExp), function(src) {
                        ...
                        if (src) $templateRequest(src, !0).then(function(response) {
                            ...
                            if (thisChangeId === changeCounter) cleanupLastIncludeContent(), scope.$emit("$includeContentError", src);
                        }), scope.$emit("$includeContentRequested", src); else cleanupLastIncludeContent(), 
                        ctrl.template = null;
                    });
                };
            }
        ...
        return {
            ...
            link: function(scope, $element, $attr, ctrl) {
                ...
                }); else return $element.html(ctrl.template), void $compile($element.contents())(scope);
            }
        };
            ...
            link: function(scope, element, attr, ctrl) {
                var ngList = element.attr(attr.$attr.ngList) || ", ", trimValues = "false" !== attr.ngTrim, separator = trimValues ? trim(ngList) : ngList, parse = function(viewValue) {
                    if (!isUndefined(viewValue)) {
                        var list = [];
                        if (viewValue) forEach(viewValue.split(separator), function(value) {
                ...
                ctrl.$parsers.push(parse), ctrl.$formatters.push(function(value) {
                    if (isArray(value)) return value.join(ngList); else return undefined;
                }), ctrl.$isEmpty = function(value) {
        ...
        this.$$setOptions = function(options) {
            if (ctrl.$options = options, options && options.getterSetter) {
                ...
                ngModelGet = function($scope) {
                    ...
                    if (isFunction(modelValue)) modelValue = invokeModelGetter($scope);
                    return modelValue;
                }, ngModelSet = function($scope) {
                    if (isFunction(parsedNgModel($scope))) invokeModelSetter($scope, {
                        $$$p: ctrl.$modelValue
        ...
        addSetValidityMethod({
            ...
            if (!isNumber(ctrl.$modelValue) || !isNaN(ctrl.$modelValue)) {
                ...
                ctrl.$$runValidators(parserValid, modelValue, viewValue, function(allValid) {
                    if (!allowInvalid && prevValid !== allValid) if (ctrl.$modelValue = allValid ? modelValue : undefined, 
            ...
            function processAsyncValidators() {
                ...
                if (forEach(ctrl.$asyncValidators, function(validator, name) {
                ...
                }), !validatorPromises.length) validationDone(!0); else $q.all(validatorPromises).then(function() {
                    validationDone(allValid);
                }, noop);
            }
            function setValidity(name, isValid) {
                if (localValidationRunId === currentValidationRunId) ctrl.$setValidity(name, isValid);
            }
            function validationDone(allValid) {
                if (localValidationRunId === currentValidationRunId) doneCallback(allValid);
            }
            currentValidationRunId++;
            var localValidationRunId = currentValidationRunId;
            if (!processParseErrors(parseValid)) return void validationDone(!1);
            if (!processSyncValidators()) return void validationDone(!1); else return void processAsyncValidators();
        }, this.$commitViewValue = function() {
            var viewValue = ctrl.$viewValue;
            if ($timeout.cancel(pendingDebounce), ctrl.$$lastCommittedViewValue !== viewValue || "" === viewValue && ctrl.$$hasNativeValidators) {
                if (ctrl.$$lastCommittedViewValue = viewValue, ctrl.$pristine) this.$setDirty();
                this.$$parseAndValidate();
            }
        }, this.$$parseAndValidate = function() {
            function writeToModelIfNeeded() {
                if (ctrl.$modelValue !== prevModelValue) ctrl.$$writeModelToScope();
            }
            ...
            if (ctrl.$$rawModelValue = modelValue, allowInvalid) ctrl.$modelValue = modelValue, 
            writeToModelIfNeeded();
            ctrl.$$runValidators(parserValid, modelValue, ctrl.$$lastCommittedViewValue, function(allValid) {
                if (!allowInvalid) ctrl.$modelValue = allValid ? modelValue : undefined, writeToModelIfNeeded();
            });
            ...
            if (ctrl.$viewValue = value, !ctrl.$options || ctrl.$options.updateOnDefault) ctrl.$$debounceViewValueCommit(trigger);
        }, this.$$debounceViewValueCommit = function(trigger) {
            var debounce, debounceDelay = 0, options = ctrl.$options;
            if (options && isDefined(options.debounce)) if (debounce = options.debounce, isNumber(debounce)) debounceDelay = debounce; else if (isNumber(debounce[trigger])) debounceDelay = debounce[trigger]; else if (isNumber(debounce["default"])) debounceDelay = debounce["default"];
            if ($timeout.cancel(pendingDebounce), debounceDelay) pendingDebounce = $timeout(function() {
                ctrl.$commitViewValue();
            }, debounceDelay); else if ($rootScope.$$phase) ctrl.$commitViewValue(); else $scope.$apply(function() {
                ctrl.$commitViewValue();
            });
                ...
                return element.addClass(PRISTINE_CLASS).addClass(UNTOUCHED_CLASS).addClass(VALID_CLASS), 
                {
                    pre: function(scope, element, attr, ctrls) {
                        ...
                        attr.$observe("name", function(newValue) {
                            if (modelCtrl.$name !== newValue) formCtrl.$$renameControl(modelCtrl, newValue);
                    ...
                    post: function(scope, element, attr, ctrls) {
                        ...
                        if (modelCtrl.$options && modelCtrl.$options.updateOn) element.on(modelCtrl.$options.updateOn, function(ev) {
                            modelCtrl.$$debounceViewValueCommit(ev && ev.type);
                        });
                        element.on("blur", function() {
                            if (!modelCtrl.$touched) if ($rootScope.$$phase) scope.$evalAsync(modelCtrl.$setTouched); else scope.$apply(modelCtrl.$setTouched);
                        });
                    }
                };
            }
        ...
        return {
            ...
            controller: [ "$scope", "$attrs", function($scope, $attrs) {
                var that = this;
                if (this.$options = $scope.$eval($attrs.ngModelOptions), this.$options.updateOn !== undefined) this.$options.updateOnDefault = !1, 
        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                forEach(attr, function(expression, attributeName) {
                    ...
                    if (!(count === lastCount || countIsNaN && isNaN(lastCount))) watchRemover(), watchRemover = scope.$watch(whensExpFns[count], updateElementText), 
                    lastCount = count;
                });
            }
        };
        ...
        var NG_REMOVED = "$$NG_REMOVED", ngRepeatMinErr = minErr("ngRepeat"), updateScope = function(scope, index, valueIdentifier, value, keyIdentifier, key, arrayLength) {
            if (scope[valueIdentifier] = value, keyIdentifier) scope[keyIdentifier] = key;
            scope.$index = index, scope.$first = 0 === index, scope.$last = index === arrayLength - 1, 
            scope.$middle = !(scope.$first || scope.$last), scope.$odd = !(scope.$even = 0 === (1 & index));
        ...
        return {
            ...
            compile: function($element, $attr) {
                ...
                if (trackByExp) trackByExpGetter = $parse(trackByExp); else trackByIdArrayFn = function(key, value) {
                    return hashKey(value);
                }, trackByIdObjFn = function(key) {
                    return key;
                };
                return function($scope, $element, $attr, ctrl, $transclude) {
                    if (trackByExpGetter) trackByIdExpFn = function(key, value, index) {
                        if (keyIdentifier) hashFnLocals[keyIdentifier] = key;
                        return hashFnLocals[valueIdentifier] = value, hashFnLocals.$index = index, trackByExpGetter($scope, hashFnLocals);
                    };
                    ...
                    $scope.$watchCollection(rhs, function(collection) {
                        ...
                        if (isArrayLike(collection)) collectionKeys = collection, trackByIdFn = trackByIdExpFn || trackByIdArrayFn; else {
                            ...
                            for (var itemKey in collection) if (collection.hasOwnProperty(itemKey) && "$" != itemKey.charAt(0)) collectionKeys.push(itemKey);
                            collectionKeys.sort();
                        }
                        ...
                        delete lastBlockMap[trackById], nextBlockMap[trackById] = block, nextBlockOrder[index] = block; else if (nextBlockMap[trackById]) throw forEach(nextBlockOrder, function(block) {
                            if (block && block.scope) lastBlockMap[block.id] = block;
                        }), ngRepeatMinErr("dupes", "Duplicates in a repeater are not allowed. Use 'track by' expression to specify unique keys. Repeater: {0}, Duplicate key: {1}, Duplicate value: {2}", expression, trackById, value); else nextBlockOrder[index] = {
                            id: trackById,
                            scope: undefined,
                            clone: undefined
                        ...
                        value = collection[key], block = nextBlockOrder[index], block.scope) {
                            ...
                            if (getBlockStart(block) != nextNode) $animate.move(getBlockNodes(block.clone), null, jqLite(previousNode));
        ...
        scope.$watchCollection(attr.ngStyle, function(newStyles, oldStyles) {
            if (oldStyles && newStyles !== oldStyles) forEach(oldStyles, function(val, style) {
                element.css(style, "");
            });
        ...
        return {
            ...
            link: function(scope, element, attr, ngSwitchController) {
                ...
                scope.$watch(watchExpr, function(value) {
                    ...
                    if (selectedElements.length = 0, selectedScopes.length = 0, selectedTranscludes = ngSwitchController.cases["!" + value] || ngSwitchController.cases["?"]) forEach(selectedTranscludes, function(selectedTransclude) {
        ...
        return {
            ...
            controller: [ "$element", "$scope", "$attrs", function($element, $scope, $attrs) {
                ...
                self.databound = $attrs.ngModel, self.init = function(ngModelCtrl_, nullOption_, unknownOption_) {
                    ...
                    if (assertNotHasOwnProperty(value, '"option value"'), optionsMap[value] = !0, ngModelCtrl.$viewValue == value) if ($element.val(value), 
                    unknownOption.parent()) unknownOption.remove();
                    if (element && element[0].hasAttribute("selected")) element[0].selected = !0;
                }, self.removeOption = function(value) {
                    if (this.hasOption(value)) if (delete optionsMap[value], ngModelCtrl.$viewValue === value) this.renderUnknownOption(value);
            ...
            link: function(scope, element, attr, ctrls) {
                function setupAsSingle(scope, selectElement, ngModelCtrl, selectCtrl) {
                    ngModelCtrl.$render = function() {
                        ...
                        if (selectCtrl.hasOption(viewValue)) {
                            ...
                            if (selectElement.val(viewValue), "" === viewValue) emptyOption.prop("selected", !0);
                        } else if (isUndefined(viewValue) && emptyOption) selectElement.val(""); else selectCtrl.renderUnknownOption(viewValue);
                ...
                function setupAsMultiple(scope, selectElement, ctrl) {
                    ...
                    ctrl.$render = function() {
                        ...
                        if (!equals(lastView, ctrl.$viewValue)) lastView = shallowCopy(ctrl.$viewValue), 
                        ctrl.$render();
                    }), selectElement.on("change", function() {
                        scope.$apply(function() {
                            var array = [];
                ...
                function setupAsOptions(scope, selectElement, ctrl) {
                    function callExpression(exprFn, key, value) {
                        if (locals[valueName] = value, keyName) locals[keyName] = key;
                    ...
                    function getViewValue(key, value) {
                        if ("?" === key) return undefined; else if ("" === key) return null; else {
                    ...
                    function getLabels() {
                        ...
                        if (values && isArray(values)) {
                            ...
                            toDisplay = {};
                            for (var prop in values) if (values.hasOwnProperty(prop)) toDisplay[prop] = callExpression(displayFn, prop, values[prop]);
                        }
                        return toDisplay;
                    }
                    function createIsSelectedFn(viewValue) {
                        ...
                        } else selectedSet = new HashMap(viewValue); else if (trackFn) viewValue = callExpression(trackFn, null, viewValue);
                        return function(key, value) {
                            var compareValueFn;
                            if (trackFn) compareValueFn = trackFn; else if (selectAsFn) compareValueFn = selectAsFn; else compareValueFn = valueFn;
                    ...
                    function scheduleRendering() {
                        if (!renderScheduled) scope.$$postDigest(render), renderScheduled = !0;
                    }
                    ...
                    function render() {
                        ...
                        for (trackKeysCache = {}, index = 0; length = keys.length, length > index; index++) {
                            if (key = index, keyName) if (key = keys[index], "$" === key.charAt(0)) continue;
                            if (value = values[key], optionGroupName = callExpression(groupByFn, key, value) || "", 
                            !(optionGroup = optionGroups[optionGroupName])) optionGroup = optionGroups[optionGroupName] = [], 
                            optionGroupNames.push(optionGroupName);
                            if (selected = isSelected(key, value), anySelected = anySelected || selected, label = callExpression(displayFn, key, value), 
                            label = isDefined(label) ? label : "", optionId = trackFn ? trackFn(scope, locals) : keyName ? keys[index] : index, 
                        ...
                        if (!multiple) if (nullOption || null === viewValue) optionGroups[""].unshift({
                            id: "",
                            label: "",
                            selected: !anySelected
                        }); else if (!anySelected) optionGroups[""].unshift({
                            id: "?",
                            label: "",
                            selected: !0
                        });
                        for (groupIndex = 0, groupLength = optionGroupNames.length; groupLength > groupIndex; groupIndex++) {
                            if (optionGroupName = optionGroupNames[groupIndex], optionGroup = optionGroups[optionGroupName], 
                            ...
                            selectElement.append(existingParent.element); else if (existingOptions = optionGroupsCache[groupIndex], 
                            ...
                            for (lastElement = null, index = 0, length = optionGroup.length; length > index; index++) if (option = optionGroup[index], 
                            existingOption = existingOptions[index + 1]) {
                                if (lastElement = existingOption.element, existingOption.label !== option.label) updateLabelMap(labelMap, existingOption.label, !1), 
                                ...
                                if (lastElement[0].selected !== option.selected) if (lastElement.prop("selected", existingOption.selected = option.selected), 
                                msie) lastElement.prop("selected", existingOption.selected);
                            } else {
                                if ("" === option.id && nullOption) element = nullOption; else (element = optionTemplate.clone()).val(option.id).prop("selected", option.selected).attr("selected", option.selected).prop("label", option.label).text(option.label);
                                if (existingOptions.push(existingOption = {
                                    element: element,
                                    label: option.label,
                                    id: option.id,
                                    selected: option.selected
                                }), updateLabelMap(labelMap, option.label, !0), lastElement) lastElement.after(element); else existingParent.element.append(element);
                                lastElement = element;
                            }
                        ...
                        forEach(labelMap, function(count, label) {
                            if (count > 0) selectCtrl.addOption(label); else if (0 > count) selectCtrl.removeOption(label);
                        });
                    }
                    var match;
                    ...
                    if (nullOption) $compile(nullOption)(scope), nullOption.removeClass("ng-scope"), 
                    nullOption.remove();
                    if (selectElement.empty(), selectElement.on("change", selectionChanged), ctrl.$render = render, 
                    scope.$watchCollection(valuesFn, scheduleRendering), scope.$watchCollection(getLabels, scheduleRendering), 
                ...
                if (ctrls[1]) {
                    ...
                    if (selectCtrl.init(ngModelCtrl, nullOption, unknownOption), multiple) ngModelCtrl.$isEmpty = function(value) {
        ...
        return {
            ...
            compile: function(element, attr) {
                if (isUndefined(attr.value)) {
                    ...
                    if (!interpolateFn) attr.$set("value", element.text());
                }
                return function(scope, element, attr) {
                    var selectCtrlName = "$selectController", parent = element.parent(), selectCtrl = parent.data(selectCtrlName) || parent.parent().data(selectCtrlName);
                    if (!selectCtrl || !selectCtrl.databound) selectCtrl = nullSelectCtrl;
                    if (interpolateFn) scope.$watch(interpolateFn, function(newVal, oldVal) {
                        if (attr.$set("value", newVal), oldVal !== newVal) selectCtrl.removeOption(oldVal);
                        selectCtrl.addOption(newVal, element);
                    }); else selectCtrl.addOption(attr.value, element);
        ...
        return {
            ...
            link: function(scope, elm, attr, ctrl) {
                if (ctrl) attr.required = !0, ctrl.$validators.required = function(modelValue, viewValue) {
    ...
    if (window.angular.bootstrap) return void console.log("WARNING: Tried to load angular more than once."); else return bindJQuery(), 
...
function(window, angular, undefined) {
    ...
    function jsonStringToDate(string) {
        ...
        if (match = string.match(R_ISO8061_STR)) {
            ...
            if (match[9]) tzHour = int(match[9] + match[10]), tzMin = int(match[9] + match[11]);
            return date.setUTCFullYear(int(match[1]), int(match[2]) - 1, int(match[3])), date.setUTCHours(int(match[4] || 0) - tzHour, int(match[5] || 0) - tzMin, int(match[6] || 0), int(match[7] || 0)), 
            date;
        }
        return string;
    }
    ...
    function createHttpBackendMock($rootScope, $timeout, $delegate, $browser) {
        function createResponse(status, data, headers, statusText) {
            if (angular.isFunction(status)) return status; else return function() {
        ...
        function $httpBackend(method, url, data, callback, headers, timeout, withCredentials) {
            ...
            function wrapResponse(wrapped) {
                ...
                if (!$browser && timeout) timeout.then ? timeout.then(handleTimeout) : $timeout(handleTimeout, timeout);
            ...
            for (var definition, i = -1; definition = definitions[++i]; ) if (definition.match(method, url, data, headers || {})) {
                if (definition.response) ($browser ? $browser.defer : responsesPush)(wrapResponse(definition)); else if (definition.passThrough) $delegate(method, url, data, callback, headers, timeout, withCredentials); else throw new Error("No response defined !");
                return;
            }
        ...
        return $httpBackend.when = function(method, url, data, headers) {
            ...
            if ($browser) chain.passThrough = function() {
                return definition.response = undefined, definition.passThrough = !0, chain;
            };
            return definitions.push(definition), chain;
            ...
            if (digest !== !1) $rootScope.$digest();
            if (expectations.length) throw new Error("Unsatisfied requests: " + expectations.join(", "));
    ...
    function MockHttpExpectation(method, url, data, headers) {
        this.data = data, this.headers = headers, this.match = function(m, u, d, h) {
            if (method != m) return !1;
            if (!this.matchUrl(u)) return !1;
            if (angular.isDefined(d) && !this.matchData(d)) return !1;
            if (angular.isDefined(h) && !this.matchHeaders(h)) return !1; else return !0;
        }, this.matchUrl = function(u) {
            if (!url) return !0;
            if (angular.isFunction(url.test)) return url.test(u);
            if (angular.isFunction(url)) return url(u); else return url == u;
        }, this.matchHeaders = function(h) {
            if (angular.isUndefined(headers)) return !0;
            if (angular.isFunction(headers)) return headers(h); else return angular.equals(headers, h);
        }, this.matchData = function(d) {
            if (angular.isUndefined(data)) return !0;
            if (data && angular.isFunction(data.test)) return data.test(d);
            if (data && angular.isFunction(data)) return data(d);
            if (data && !angular.isString(data)) return angular.equals(angular.fromJson(angular.toJson(data)), angular.fromJson(d)); else return data == d;
    ...
    function MockXhr() {
        MockXhr.$$lastInstance = this, this.open = function(method, url, async) {
            ...
            if (name = angular.lowercase(name), header = this.$$respHeaders[name]) return header; else return header = undefined, 
    ...
    angular.mock = {}, angular.mock.$BrowserProvider = function() {
        ...
        self.onUrlChange = function(listener) {
            return self.pollFns.push(function() {
                if (self.$$lastUrl !== self.$$url || self.$$state !== self.$$lastState) self.$$lastUrl = self.$$url, 
        ...
        self.deferredFns = [], self.deferredNextId = 0, self.defer = function(fn, delay) {
            ...
            var fnIndex;
            if (angular.forEach(self.deferredFns, function(fn, index) {
                if (fn.id === deferId) fnIndex = index;
            }), fnIndex !== undefined) return self.deferredFns.splice(fnIndex, 1), !0; else return !1;
        }, self.defer.flush = function(delay) {
            if (angular.isDefined(delay)) self.defer.now += delay; else if (self.deferredFns.length) self.defer.now = self.deferredFns[self.deferredFns.length - 1].time; else throw new Error("No deferred tasks to be flushed");
        ...
        url: function(url, replace, state) {
            ...
            if (url) return this.$$url = url, this.$$state = angular.copy(state), this; else return this.$$url;
        },
        state: function() {
            return this.$$state;
        },
        cookies: function(name, value) {
            if (name) {
                if (angular.isUndefined(value)) delete this.cookieHash[name]; else if (angular.isString(value) && value.length <= 4096) this.cookieHash[name] = value;
            } else {
                if (!angular.equals(this.cookieHash, this.lastCookieHash)) this.lastCookieHash = angular.copy(this.cookieHash), 
        ...
        this.mode = function(mode) {
            switch (mode) {
              ...
              case "rethrow":
                var errors = [];
                handler = function(e) {
                    if (1 == arguments.length) errors.push(e); else errors.push([].slice.call(arguments, 0));
                    if ("rethrow" === mode) throw e;
        ...
        var debug = !0;
        this.debugEnabled = function(flag) {
            if (angular.isDefined(flag)) return debug = flag, this; else return debug;
        }, this.$get = function() {
            var $log = {
                log: function() {
                ...
                debug: function() {
                    if (debug) $log.debug.logs.push(concat([], arguments, 0));
                }
            };
        ...
        this.$get = [ "$browser", "$rootScope", "$q", "$$q", function($browser, $rootScope, $q, $$q) {
            var repeatFns = [], nextRepeatId = 0, now = 0, $interval = function(fn, delay, count, invokeApply) {
                function tick() {
                    if (deferred.notify(iteration++), count > 0 && iteration >= count) {
                        var fnIndex;
                        if (deferred.resolve(iteration), angular.forEach(repeatFns, function(fn, index) {
            ...
            return $interval.cancel = function(promise) {
                if (!promise) return !1;
                var fnIndex;
                if (angular.forEach(repeatFns, function(fn, index) {
                    if (fn.id === promise.$$intervalId) fnIndex = index;
                }), fnIndex !== undefined) return repeatFns[fnIndex].deferred.reject("canceled"), 
                repeatFns.splice(fnIndex, 1), !0; else return !1;
    ...
    if (angular.mock.TzDate = function(offset, timestamp) {
        ...
        if (self.offsetDiff = 60 * localOffset * 1e3 - 1e3 * offset * 60 * 60, self.date = new Date(timestamp + self.offsetDiff), 
        ...
        }), $provide.decorator("$animate", [ "$delegate", "$$asyncCallback", "$timeout", "$browser", function($delegate, $$asyncCallback, $timeout) {
            var animate = {
                queue: [],
        ...
        function serialize(object) {
            var out;
            if (angular.isElement(object)) object = angular.element(object), out = angular.element("<div></div>"), 
            angular.forEach(object, function(element) {
                out.append(angular.element(element).clone());
            }), out = out.html(); else if (angular.isArray(object)) out = [], angular.forEach(object, function(o) {
                out.push(serialize(o));
            }), out = "[ " + out.join(", ") + " ]"; else if (angular.isObject(object)) if (angular.isFunction(object.$eval) && angular.isFunction(object.$apply)) out = serializeScope(object); else if (object instanceof Error) out = object.stack || "" + object.name + ": " + object.message; else out = angular.toJson(object, !0); else out = String(object);
            return out;
        }
        function serializeScope(scope, offset) {
            offset = offset || "  ";
            var log = [ offset + "Scope(" + scope.$id + "): {" ];
            for (var key in scope) if (Object.prototype.hasOwnProperty.call(scope, key) && !key.match(/^(\$|this)/)) log.push("  " + key + ": " + angular.toJson(scope[key]));
    ...
    } ], angular.mock.$AsyncCallbackDecorator = [ "$delegate", function() {
    ...
    angular.mock.$RootScopeDecorator = [ "$delegate", function($delegate) {
        ...
        angular.mock.$$annotate = angular.injector.$$annotate, angular.injector.$$annotate = function(fn) {
            if ("function" == typeof fn && !fn.$inject) annotatedFunctions.push(fn);
            ...
            if (annotatedFunctions.forEach(function(fn) {
                ...
                if (module && module.$$hashKey) module.$$hashKey = undefined;
            ...
            function workFn() {
                if (currentSpec.$injector) throw new Error("Injector already created, can not register a module!"); else {
                    var modules = currentSpec.$modules || (currentSpec.$modules = []);
                    angular.forEach(moduleFns, function(module) {
                        if (angular.isObject(module) && !angular.isArray(module)) modules.push(function($provide) {
        ...
        var ErrorAddingDeclarationLocationStack = function(e, errorForStack) {
            if (this.message = e.message, this.name = e.name, e.line) this.line = e.line;
        ...
        window.inject = angular.mock.inject = function() {
            function workFn() {
                ...
                var injector = currentSpec.$injector;
                if (!injector) {
                    if (strictDi) angular.forEach(modules, function(moduleFn) {
                        if ("function" == typeof moduleFn) angular.injector.$$annotate(moduleFn);
                    });
                ...
                for (var i = 0, ii = blockFns.length; ii > i; i++) {
                    if (currentSpec.$injectorStrict) injector.annotate(blockFns[i]);
                    try {
            ...
            function workFn() {
                if (value !== currentSpec.$injectorStrict) if (currentSpec.$injector) throw new Error("Injector already created, can not modify strict annotations"); else currentSpec.$injectorStrict = value;
            }
    ...
    function $RouteProvider() {
        ...
        this.when = function(path, route) {
            ...
            if (routes[path] = angular.extend(routeCopy, path && pathRegExp(path, routeCopy)), 
            path) {
            ...
            function prepareRoute($locationEvent) {
                ...
                if (preparedRoute = parseRoute(), preparedRouteIsUpdateOnly = preparedRoute && lastRoute && preparedRoute.$$route === lastRoute.$$route && angular.equals(preparedRoute.pathParams, lastRoute.pathParams) && !preparedRoute.reloadOnSearch && !forceReload, 
                !preparedRouteIsUpdateOnly && (lastRoute || preparedRoute)) if ($rootScope.$broadcast("$routeChangeStart", preparedRoute, lastRoute).defaultPrevented) if ($locationEvent) $locationEvent.preventDefault();
            }
            function commitRoute() {
                ...
                $rootScope.$broadcast("$routeUpdate", lastRoute); else if (nextRoute || lastRoute) {
                    if (forceReload = !1, $route.current = nextRoute, nextRoute) if (nextRoute.redirectTo) if (angular.isString(nextRoute.redirectTo)) $location.path(interpolate(nextRoute.redirectTo, nextRoute.params)).search(nextRoute.params).replace(); else $location.url(nextRoute.redirectTo(nextRoute.pathParams, $location.path(), $location.search())).replace();
                    $q.when(nextRoute).then(function() {
                        if (nextRoute) {
                            var template, templateUrl, locals = angular.extend({}, nextRoute.resolve);
                            if (angular.forEach(locals, function(value, key) {
                                locals[key] = angular.isString(value) ? $injector.get(value) : $injector.invoke(value, null, null, key);
                            }), angular.isDefined(template = nextRoute.template)) {
                                if (angular.isFunction(template)) template = template(nextRoute.params);
                            } else if (angular.isDefined(templateUrl = nextRoute.templateUrl)) {
                                if (angular.isFunction(templateUrl)) templateUrl = templateUrl(nextRoute.params);
                                if (templateUrl = $sce.getTrustedResourceUrl(templateUrl), angular.isDefined(templateUrl)) nextRoute.loadedTemplateUrl = templateUrl, 
                        ...
                        if (nextRoute == $route.current) {
                            if (nextRoute) nextRoute.locals = locals, angular.copy(nextRoute.params, $routeParams);
                            $rootScope.$broadcast("$routeChangeSuccess", nextRoute, lastRoute);
                        }
                    }, function(error) {
                        if (nextRoute == $route.current) $rootScope.$broadcast("$routeChangeError", nextRoute, lastRoute, error);
                    });
                }
            }
            function parseRoute() {
                ...
                return angular.forEach(routes, function(route) {
                    if (!match && (params = switchRouteMatcher($location.path(), route))) match = inherit(route, {
            ...
            var preparedRoute, preparedRouteIsUpdateOnly, forceReload = !1, $route = {
                ...
                updateParams: function(newParams) {
                    if (this.current && this.current.$$route) newParams = angular.extend({}, this.current.params, newParams), 
                    $location.path(interpolate(this.current.$$route.originalPath, newParams)), $location.search(newParams); else throw $routeMinErr("norout", "Tried updating route when with no current route");
                }
            };
        ...
        return {
            ...
            link: function(scope, $element, attr, ctrl, $transclude) {
                function cleanupLastView() {
                    if (previousLeaveAnimation) $animate.cancel(previousLeaveAnimation), previousLeaveAnimation = null;
                    if (currentScope) currentScope.$destroy(), currentScope = null;
                    if (currentElement) previousLeaveAnimation = $animate.leave(currentElement), previousLeaveAnimation.then(function() {
                    ...
                    if (angular.isDefined(template)) {
                        var newScope = scope.$new(), current = $route.current, clone = $transclude(newScope, function(clone) {
                            $animate.enter(clone, null, currentElement || $element).then(function() {
                                if (angular.isDefined(autoScrollExp) && (!autoScrollExp || scope.$eval(autoScrollExp))) $anchorScroll();
                            }), cleanupLastView();
                        });
