if (function(window, angular, undefined) {
    "use strict";
    angular.module("ngAnimate", [ "ng" ]).directive("ngAnimateChildren", function() {
        var NG_ANIMATE_CHILDREN = "$$ngAnimateChildren";
        return function(scope, element, attrs) {
            var val = attrs.ngAnimateChildren;
            if (angular.isString(val) && 0 === val.length) element.data(NG_ANIMATE_CHILDREN, !0); else scope.$watch(val, function(value) {
        ...
        $provide.decorator("$animate", [ "$delegate", "$$q", "$injector", "$sniffer", "$rootElement", "$$asyncCallback", "$rootScope", "$document", "$templateRequest", "$$jqLite", function($delegate, $$q, $injector, $sniffer, $rootElement, $$asyncCallback, $rootScope, $document, $templateRequest, $$$jqLite) {
            function classBasedAnimationsBlocked(element, setter) {
                ...
                if (setter) data.running = !0, data.structural = !0, element.data(NG_ANIMATE_STATE, data);
            ...
            function parseAnimateOptions(options) {
                if (isObject(options)) {
                    if (options.tempClasses && isString(options.tempClasses)) options.tempClasses = options.tempClasses.split(/\s+/);
                    return options;
                }
            }
            function resolveElementClasses(element, cache, runningAnimations) {
                ...
                return forEach(cache && cache.classes || [], function(status, className) {
                    ...
                    if (status === !1) {
                        if (hasClass || "addClass" == matchingAnimation.event) toRemove.push(className);
                    } else if (status === !0) if (!hasClass || "removeClass" == matchingAnimation.event) toAdd.push(className);
                }), toAdd.length + toRemove.length > 0 && [ toAdd.join(" "), toRemove.join(" ") ];
            }
            function lookup(name) {
                if (name) {
                    var matches = [], flagMap = {}, classes = name.substr(1).split(".");
                    if ($sniffer.transitions || $sniffer.animations) matches.push($injector.get(selectors[""]));
                    for (var i = 0; i < classes.length; i++) {
                        var klass = classes[i], selectorFactoryName = selectors[klass];
                        if (selectorFactoryName && !flagMap[klass]) matches.push($injector.get(selectorFactoryName)), 
                        flagMap[klass] = !0;
                    }
                    return matches;
                }
            ...
            function animationRunner(element, animationEvent, className, options) {
                function registerAnimation(animationFactory, event) {
                    ...
                    if (afterFn || beforeFn) {
                        if ("leave" == event) beforeFn = afterFn, afterFn = null;
                        return after.push({
                            event: event,
                            fn: afterFn
                        }), before.push({
                ...
                function run(fns, cancellations, allCompleteFn) {
                    ...
                    var count = 0;
                    if (forEach(animations, function(animation, index) {
                ...
                if (node) {
                    ...
                    if (isArray(className)) if (classNameAdd = className[0], classNameRemove = className[1], 
                    !classNameAdd) className = classNameRemove, animationEvent = "removeClass"; else if (!classNameRemove) className = classNameAdd, 
                    animationEvent = "addClass"; else className = classNameAdd + " " + classNameRemove;
                    ...
                    if (isAnimatableClassName(classes)) {
                        ...
                        return forEach(lookup(animationLookup), function(animationFactory) {
                            ...
                            if (!created && isSetClassOperation) registerAnimation(animationFactory, "addClass"), 
                            ...
                            applyStyles: function() {
                                if (options) element.css(angular.extend(options.from || {}, options.to || {}));
                            },
                            ...
                            cancel: function() {
                                if (beforeCancel) forEach(beforeCancel, function(cancelFn) {
                                    (cancelFn || noop)(!0);
                                }), beforeComplete(!0);
                                if (afterCancel) forEach(afterCancel, function(cancelFn) {
                                    (cancelFn || noop)(!0);
                                }), afterComplete(!0);
                            }
                        };
            ...
            function performAnimation(animationEvent, className, element, parentElement, afterElement, domOperation, options, doneCallback) {
                function fireDOMCallback(animationPhase) {
                    ...
                    if (elementEvents && elementEvents[eventName] && elementEvents[eventName].length > 0) $$asyncCallback(function() {
                ...
                function fireDOMOperation() {
                    if (!fireDOMOperation.hasBeenRun) fireDOMOperation.hasBeenRun = !0, domOperation();
                }
                function closeAnimation() {
                    if (!closeAnimation.hasBeenRun) {
                        if (runner) runner.applyStyles();
                        if (closeAnimation.hasBeenRun = !0, options && options.tempClasses) forEach(options.tempClasses, function(className) {
                            $$jqLite.removeClass(element, className);
                        });
                        var data = element.data(NG_ANIMATE_STATE);
                        if (data) if (runner && runner.isClassBased) cleanup(element, className); else $$asyncCallback(function() {
                            var data = element.data(NG_ANIMATE_STATE) || {};
                            if (localAnimationCount == data.index) cleanup(element, className, animationEvent);
                ...
                if (animationsDisabled(element, parentElement)) return fireDOMOperation(), fireBeforeCallbackAsync(), 
                ...
                if (totalActiveAnimations > 0) {
                    ...
                    if (!runner.isClassBased) if ("leave" == animationEvent && runningAnimations["ng-leave"]) skipAnimation = !0; else {
                        for (var klass in runningAnimations) animationsToCancel.push(runningAnimations[klass]);
                        ngAnimateState = {}, cleanup(element, !0);
                    } else if ("setClass" == lastAnimation.event) animationsToCancel.push(lastAnimation), 
                    cleanup(element, className); else if (runningAnimations[className]) {
                        var current = runningAnimations[className];
                        if (current.event == animationEvent) skipAnimation = !0; else animationsToCancel.push(current), 
                        cleanup(element, className);
                    }
                    if (animationsToCancel.length > 0) forEach(animationsToCancel, function(operation) {
                        operation.cancel();
                    });
                }
                ...
                if (skipAnimation) return fireDOMOperation(), fireBeforeCallbackAsync(), fireAfterCallbackAsync(), 
                ...
                "leave" == animationEvent) element.one("$destroy", function() {
                    ...
                    if (state) {
                        ...
                        if (activeLeaveAnimation) activeLeaveAnimation.cancel(), cleanup(element, "ng-leave");
                    }
                });
                if ($$jqLite.addClass(element, NG_ANIMATE_CLASS_NAME), options && options.tempClasses) forEach(options.tempClasses, function(className) {
                ...
                return totalActiveAnimations++, runningAnimations[className] = runner, element.data(NG_ANIMATE_STATE, {
                    ...
                    var data = element.data(NG_ANIMATE_STATE);
                    if (cancelled = cancelled || !data || !data.active[className] || runner.isClassBased && data.active[className].event != animationEvent, 
                    fireDOMOperation(), cancelled === !0) closeAnimation(); else fireAfterCallbackAsync(), 
            ...
            function cancelChildAnimations(element) {
                ...
                if (node) {
                    ...
                    forEach(nodes, function(element) {
                        ...
                        var data = element.data(NG_ANIMATE_STATE);
                        if (data && data.active) forEach(data.active, function(runner) {
                            runner.cancel();
                        });
                    });
                }
            ...
            function cleanup(element, className) {
                if (isMatchingElement(element, $rootElement)) {
                    if (!rootAnimateState.disabled) rootAnimateState.running = !1, rootAnimateState.structural = !1;
                } else if (className) {
                    var data = element.data(NG_ANIMATE_STATE) || {}, removeAnimations = className === !0;
                    if (!removeAnimations && data.active && data.active[className]) data.totalActive--, 
                    delete data.active[className];
                    if (removeAnimations || !data.totalActive) $$jqLite.removeClass(element, NG_ANIMATE_CLASS_NAME), 
            ...
            function animationsDisabled(element, parentElement) {
                ...
                do {
                    ...
                    if (allowChildAnimations !== !1) {
                        var animateChildrenFlag = parentElement.data(NG_ANIMATE_CHILDREN);
                        if (angular.isDefined(animateChildrenFlag)) allowChildAnimations = animateChildrenFlag;
                    }
            ...
            var deregisterWatch = $rootScope.$watch(function() {
                ...
                if (0 === val) deregisterWatch(), $rootScope.$$postDigest(function() {
                    $rootScope.$$postDigest(function() {
                        rootAnimateState.running = !1;
                    });
                });
            }), globalAnimationCounter = 0, classNameFilter = $animateProvider.classNameFilter(), isAnimatableClassName = !classNameFilter ? function() {
                return !0;
            } : function(className) {
            ...
            return {
                ...
                setClass: function(element, add, remove, options) {
                    ...
                    if (!cache) cache = {}, cache.classes = {};
                    if (classes = cache.classes, add = isArray(add) ? add : add.split(" "), forEach(add, function(c) {
                        if (c && c.length) classes[c] = !0;
                    }), remove = isArray(remove) ? remove : remove.split(" "), forEach(remove, function(c) {
                        if (c && c.length) classes[c] = !1;
                    }), hasCache) {
                        if (options && cache.options) cache.options = angular.extend(cache.options || {}, options);
                        return cache.promise;
                    } else element.data(STORAGE_KEY, cache = {
                        classes: classes,
                        options: options
                    });
            ...
            function clearCacheAfterReflow() {
                if (!cancelAnimationReflow) cancelAnimationReflow = $$animateReflow(function() {
                    animationReflowQueue = [], cancelAnimationReflow = null, lookupCache = {};
                });
            }
            function afterReflow(element, callback) {
                if (cancelAnimationReflow) cancelAnimationReflow();
                animationReflowQueue.push(callback), cancelAnimationReflow = $$animateReflow(function() {
            ...
            function animationCloseHandler(element, totalTime) {
                ...
                if (!(closingTimestamp >= futureTimestamp)) $timeout.cancel(closingTimer), closingTimestamp = futureTimestamp, 
            ...
            function closeAllAnimations(elements) {
                forEach(elements, function(element) {
                    ...
                    if (elementData) forEach(elementData.closeAnimationFns, function(fn) {
                        fn();
                    });
                });
            }
                ...
                if (!data) {
                    ...
                    if (forEach(element, function(element) {
                        if (element.nodeType == ELEMENT_NODE) {
                            ...
                            if (aDuration > 0) aDuration *= parseInt(elementStyles[ANIMATION_PROP + ANIMATION_ITERATION_COUNT_KEY], 10) || 1;
            ...
            function getCacheKey(element) {
                ...
                if (!parentID) parentElement.data(NG_ANIMATE_PARENT_KEY, ++parentCounter), parentID = parentCounter;
            ...
            function animateSetup(animationEvent, element, className, styles) {
                ...
                var node = extractElementNode(element);
                if (blockTransition) if (blockTransitions(node, !0), styles) element.css(styles);
                if (blockAnimation) blockAnimations(node, !0);
                return !0;
            }
            function animateRun(animationEvent, element, className, activeAnimationComplete, styles) {
                function onEnd() {
                    if (element.off(css3AnimationEvents, onAnimationProgress), $$jqLite.removeClass(element, activeClassName), 
                ...
                function onAnimationProgress(event) {
                    ...
                    if (Math.max(timeStamp - startTime, 0) >= maxDelayTime && elapsedTime >= maxDuration) activeAnimationComplete();
                }
                ...
                if (itemIndex > 0) {
                    ...
                    if (stagger.transitionDelay > 0 && 0 === stagger.transitionDuration) transitionStaggerDelay = stagger.transitionDelay * itemIndex;
                    var animationStaggerDelay = 0;
                    if (stagger.animationDelay > 0 && 0 === stagger.animationDuration) animationStaggerDelay = stagger.animationDelay * itemIndex, 
                    appliedStyles.push(CSS_PREFIX + "animation-play-state");
                    staggerTime = Math.round(100 * Math.max(transitionStaggerDelay, animationStaggerDelay)) / 100;
                }
                if (!staggerTime) if ($$jqLite.addClass(element, activeClassName), elementData.blockTransition) blockTransitions(node, !1);
                ...
                if (!staggerTime && styles && Object.keys(styles).length > 0) {
                    if (!timings.transitionDuration) element.css("transition", timings.animationDuration + "s linear all"), 
                ...
                if (appliedStyles.length > 0) {
                    ...
                    if (";" !== oldStyle.charAt(oldStyle.length - 1)) oldStyle += ";";
                ...
                if (staggerTime > 0) $$jqLite.addClass(element, pendingClassName), staggerTimeout = $timeout(function() {
                    if (staggerTimeout = null, timings.transitionDuration > 0) blockTransitions(node, !1);
                    if (timings.animationDuration > 0) blockAnimations(node, !1);
                    if ($$jqLite.addClass(element, activeClassName), $$jqLite.removeClass(element, pendingClassName), 
                    styles) {
                        if (0 === timings.transitionDuration) element.css("transition", timings.animationDuration + "s linear all");
                        element.css(styles), appliedStyles.push("transition");
                    }
                }, staggerTime * ONE_SECOND, !1);
                return element.on(css3AnimationEvents, onAnimationProgress), elementData.closeAnimationFns.push(function() {
            ...
            function animateAfter(animationEvent, element, className, afterAnimationComplete, styles) {
                if (element.data(NG_ANIMATE_CSS_DATA_KEY)) return animateRun(animationEvent, element, className, afterAnimationComplete, styles); else animateClose(element, className), 
            ...
            function animateClose(element, className) {
                ...
                if (data) {
                    ...
                    if (!data.running || 0 === data.running) element.removeData(NG_ANIMATE_CSS_DATA_KEY);
                }
            }
            function suffixClasses(classes, suffix) {
                var className = "";
                return classes = isArray(classes) ? classes : classes.split(/\s+/), forEach(classes, function(klass, i) {
                    if (klass && klass.length > 0) className += (i > 0 ? " " : "") + klass + suffix;
                }), className;
            }
            var TRANSITION_PROP, TRANSITIONEND_EVENT, ANIMATION_PROP, ANIMATIONEND_EVENT, CSS_PREFIX = "";
            if (window.ontransitionend === undefined && window.onwebkittransitionend !== undefined) CSS_PREFIX = "-webkit-", 
            TRANSITION_PROP = "WebkitTransition", TRANSITIONEND_EVENT = "webkitTransitionEnd transitionend"; else TRANSITION_PROP = "transition", 
            TRANSITIONEND_EVENT = "transitionend";
            if (window.onanimationend === undefined && window.onwebkitanimationend !== undefined) CSS_PREFIX = "-webkit-", 
            ANIMATION_PROP = "WebkitAnimation", ANIMATIONEND_EVENT = "webkitAnimationEnd animationend"; else ANIMATION_PROP = "animation", 
            ...
            return {
                ...
                beforeSetClass: function(element, add, remove, animationCompleted, options) {
                    ...
                    if (cancellationMethod) return afterReflow(element, animationCompleted), cancellationMethod; else return clearCacheAfterReflow(), 
...
angular.module("ui.bootstrap.transition", []).factory("$transition", [ "$q", "$timeout", "$rootScope", function(a, b, c) {
    ...
    j.select = a.select = function(c, f) {
        function g() {
            if (!m) {
                if (j.currentSlide && angular.isString(f) && !a.noTransition && c.$element) c.$element.addClass(f), 
                ...
                }(c, j.currentSlide); else h(c, j.currentSlide);
...
!function(window, angular, undefined) {
    ...
    function merge(dst) {
        return forEach(arguments, function(obj) {
            if (obj !== dst) forEach(obj, function(value, key) {
    ...
    function indexOf(array, value) {
        ...
        if (from = 0 > from ? Math.ceil(from) : Math.floor(from), 0 > from) from += len;
        for (;len > from; from++) if (from in array && array[from] === value) return from;
        return -1;
    }
    function inheritParams(currentParams, newParams, $current, $to) {
        var parentParams, parents = ancestors($current, $to), inherited = {}, inheritList = [];
        for (var i in parents) if (parents[i].params) if (parentParams = objectKeys(parents[i].params), 
        parentParams.length) for (var j in parentParams) if (!(indexOf(inheritList, parentParams[j]) >= 0)) inheritList.push(parentParams[j]), 
        inherited[parentParams[j]] = currentParams[parentParams[j]]; else ; else ; else ;
    ...
    function omit(obj) {
        ...
        for (var key in obj) if (-1 == indexOf(keys, key)) copy[key] = obj[key];
        return copy;
    }
    function filter(collection, callback) {
        var array = isArray(collection), result = array ? [] : {};
        return forEach(collection, function(val, i) {
            if (callback(val, i)) result[array ? result.length : i] = val;
        }), result;
    }
            ...
            return forEach(invocables, visit), invocables = cycle = visited = null, function(locals, parent, self) {
                function done() {
                    if (!--wait) {
                        ...
                        result.$$values = values, result.$$promises = result.$$promises || !0, delete result.$$inheritedValues, 
                ...
                function invoke(key, invocable, params) {
                    ...
                    if (forEach(params, function(dep) {
                        if (promises.hasOwnProperty(dep) && !locals.hasOwnProperty(dep)) waitParams++, promises[dep].then(function(result) {
                            if (values[dep] = result, !--waitParams) proceed();
                        }, onfailure);
                    }), !waitParams) proceed();
                    promises[key] = invocation.promise;
                }
                if (isResolve(locals) && self === undefined) self = parent, parent = locals, locals = null;
                ...
                if (extend(promises, parent.$$promises), parent.$$values) merged = merge(values, omit(parent.$$values, invocableKeys)), 
                result.$$inheritedValues = omit(parent.$$values, invocableKeys), done(); else {
                    if (parent.$$inheritedValues) result.$$inheritedValues = omit(parent.$$inheritedValues, invocableKeys);
                    parent.then(done, fail);
                }
                for (var i = 0, ii = plan.length; ii > i; i += 3) if (locals.hasOwnProperty(plan[i])) done(); else invoke(plan[i], plan[i + 1], plan[i + 2]);
                return result;
            };
    ...
    function $TemplateFactory($http, $templateCache, $injector) {
        this.fromConfig = function(config, params, locals) {
            ...
            if (null == url) return null; else return $http.get(url, {
                cache: $templateCache,
                headers: {
                    Accept: "text/html"
                }
    ...
    function $UrlMatcherFactory() {
        ...
        var injector, isCaseInsensitive = !1, isStrictMode = !0, defaultSquashPolicy = !1, $types = {}, enqueue = !0, typeQueue = [], defaultTypes = {
            ...
            date: {
                encode: function(val) {
                    if (!this.is(val)) return undefined; else return [ val.getFullYear(), ("0" + (val.getMonth() + 1)).slice(-2), ("0" + val.getDate()).slice(-2) ].join("-");
                },
                decode: function(val) {
        ...
        $UrlMatcherFactory.$$getDefaultValue = function(config) {
            ...
            if ($types[name] = new Type(extend({
                name: name
            }, definition)), definitionFn) if (typeQueue.push({
                name: name,
                def: definitionFn
            ...
            function getType(config, urlType, location) {
                ...
                if (!config.type) return "config" === location ? $types.any : $types.string; else return config.type instanceof Type ? config.type : new Type(config.type);
            }
            ...
            function getReplace(config, arrayMode, isOptional, squash) {
                ...
                if (replace = isArray(config.replace) ? config.replace : [], isString(squash)) replace.push({
                    from: squash,
                    to: undefined
                });
            ...
            if (type = arrayMode ? type.$asArray(arrayMode, "search" === location) : type, "string" === type.name && !arrayMode && "path" === location && config.value === undefined) config.value = "";
            ...
            $$keys: function() {
                ...
                return chain.reverse(), forEach(chain, function(paramset) {
                    forEach(objectKeys(paramset), function(key) {
                        if (-1 === indexOf(keys, key) && -1 === indexOf(ignore, key)) keys.push(key);
                    });
                }), keys;
            },
            ...
            $$equals: function(paramValues1, paramValues2) {
                ...
                return forEach(self.$$keys(), function(key) {
                    ...
                    if (!self[key].type.equals(left, right)) equal = !1;
                }), equal;
            },
    ...
    function $UrlRouterProvider($locationProvider, $urlMatcherFactory) {
        ...
        function $get($location, $rootScope, $injector, $browser) {
            function appendBasePath(url, isHtml5, absolute) {
                ...
                if (absolute) return baseHref.slice(1) + url; else return url;
            }
            function update(evt) {
                function check(rule) {
            ...
            return {
                ...
                update: function(read) {
                    ...
                    if ($location.url() !== location) $location.url(location), $location.replace();
                },
                push: function(urlMatcher, params, options) {
                    if ($location.url(urlMatcher.format(params || {})), lastPushedUrl = options && options.$$avoidResync ? $location.url() : undefined, 
                    options && options.replace) $location.replace();
                },
                href: function(urlMatcher, params, options) {
                    if (!urlMatcher.validates(params)) return null;
                    var isHtml5 = $locationProvider.html5Mode();
                    if (angular.isObject(isHtml5)) isHtml5 = isHtml5.enabled;
                    var url = urlMatcher.format(params);
                    if (options = options || {}, !isHtml5 && null !== url) url = "#" + $locationProvider.hashPrefix() + url;
                    if (url = appendBasePath(url, isHtml5, options.absolute), !options.absolute || !url) return url;
        ...
        this.rule = function(rule) {
            ...
            var strategies = {
                matcher: function(what, handler) {
                    if (handlerIsString) redirect = $urlMatcherFactory.compile(handler), handler = [ "$match", function($match) {
                ...
                regex: function(what, handler) {
                    ...
                    if (handlerIsString) redirect = handler, handler = [ "$match", function($match) {
    ...
    function $StateProvider($urlRouterProvider, $urlMatcherFactory) {
        ...
        function findState(stateOrName, base) {
            ...
            if (path) {
                ...
                base = findState(base);
                for (var rel = name.split("."), i = 0, pathLength = rel.length, current = base; pathLength > i; i++) if ("" !== rel[i] || 0 !== i) if ("^" !== rel[i]) break; else {
            ...
            if (state && (isStr || !isStr && (state === stateOrName || state.self === stateOrName))) return state; else return undefined;
        }
        function queueState(parentName, state) {
            if (!queue[parentName]) queue[parentName] = [];
        ...
        function registerState(state) {
            ...
            if (states[name] = state, !state[abstractKey] && state.url) $urlRouterProvider.when(state.url, [ "$match", "$stateParams", function($match, $stateParams) {
                if ($state.$current.navigable != state || !equalForKeys($match, $stateParams)) $state.transitionTo(state, $match, {
                    inherit: !0,
                    location: !1
                });
            } ]);
            return flushQueuedChildren(name), state;
        }
        ...
        function doesStateMatchGlob(glob) {
            ...
            segments.unshift("**");
            if ("**" === globSegments[globSegments.length - 1]) segments.splice(indexOf(segments, globSegments[globSegments.length - 2]) + 1, Number.MAX_VALUE), 
            segments.push("**");
            if (globSegments.length != segments.length) return !1;
            for (var i = 0, l = globSegments.length; l > i; i++) if ("*" === globSegments[i]) segments[i] = "*";
        ...
        function $get($rootScope, $q, $view, $injector, $resolve, $stateParams, $urlRouter) {
            function handleRedirect(redirect, state, params, options) {
                ...
                return retryTransition.then(function() {
                    if (retryTransition !== $state.transition) return TransitionSuperseded; else return redirect.options.$retry = !0, 
            ...
            return root.locals = {
                ...
                if (shouldTriggerReload(to, from, locals, options)) {
                    if (to.self.reloadOnSearch !== !1) $urlRouter.update();
                    return $state.transition = null, $q.when($state.current);
                }
                if (toParams = filterByKeys(to.params.$$keys(), toParams || {}), options.notify) if ($rootScope.$broadcast("$stateChangeStart", to.self, toParams, from.self, fromParams).defaultPrevented) return $urlRouter.update(), 
                TransitionPrevented;
                ...
                var transition = $state.transition = resolved.then(function() {
                    ...
                    for (l = fromPath.length - 1; l >= keep; l--) {
                        if (exiting = fromPath[l], exiting.self.onExit) $injector.invoke(exiting.self.onExit, exiting.self, exiting.locals.globals);
                        exiting.locals = null;
                    }
                    for (l = keep; l < toPath.length; l++) if (entering = toPath[l], entering.locals = toLocals[l], 
                    entering.self.onEnter) $injector.invoke(entering.self.onEnter, entering.self, entering.locals.globals);
                    if ($state.transition !== transition) return TransitionSuperseded;
                    if ($state.$current = to, $state.current = to.self, $state.params = toParams, copy($state.params, $stateParams), 
                    ...
                    return $urlRouter.update(!0), $state.current;
                }, function(error) {
                    if ($state.transition !== transition) return TransitionSuperseded;
                    if ($state.transition = null, evt = $rootScope.$broadcast("$stateChangeError", to.self, toParams, from.self, fromParams, error), 
                ...
                if ($state.$current !== state) return !1; else return params ? equalForKeys(state.params.$$values(params), $stateParams) : !0;
                ...
                var state = findState(stateOrName, options.relative);
                if (!isDefined(state)) return undefined;
                if (!isDefined($state.$current.includes[state.name])) return !1; else return params ? equalForKeys(state.params.$$values(params), $stateParams, objectKeys(params)) : !0;
                ...
                if (!nav || nav.url === undefined || null === nav.url) return null; else return $urlRouter.href(nav.url, filterByKeys(state.params.$$keys(), params || {}), {
        ...
        function shouldTriggerReload(to, from, locals, options) {
            if (to === from && (locals === from.locals && !options.reload || to.self.reloadOnSearch === !1)) return !0; else return void 0;
        }
        var root, $state, states = {}, queue = {}, abstractKey = "abstract", stateBuilder = {
            ...
            data: function(state) {
                if (state.parent && state.parent.data) state.data = state.self.data = extend({}, state.parent.data, state.data);
                return state.data;
            },
            url: function(state) {
                ...
                if (isString(url)) if ("^" == url.charAt(0)) return $urlMatcherFactory.compile(url.substring(1), config); else return (state.parent.navigable || root).url.concat(url, config);
        ...
        function $get($rootScope, $templateFactory) {
            return {
                load: function(name, options) {
                    ...
                    if (options = extend(defaults, options), options.view) result = $templateFactory.fromConfig(options.view, options.params, options.locals);
    ...
    function $ViewScrollProvider() {
        ...
        this.useAnchorScroll = function() {
            ...
            if (useAnchorScroll) return $anchorScroll; else return function($element) {
                $timeout(function() {
            ...
            compile: function(tElement, tAttrs, $transclude) {
                return function(scope, $element, attrs) {
                    function cleanupLastView() {
                        if (previousEl) previousEl.remove(), previousEl = null;
                        if (currentScope) currentScope.$destroy(), currentScope = null;
                        if (currentEl) renderer.leave(currentEl, function() {
                            previousEl = null;
                        ...
                        if (firstTime || previousLocals !== latestLocals) {
                            ...
                            var clone = $transclude(newScope, function(clone) {
                                renderer.enter(clone, $element, function() {
                                    ...
                                    if (angular.isDefined(autoScrollExp) && !autoScrollExp || scope.$eval(autoScrollExp)) $uiViewScroll(clone);
                                }), cleanupLastView();
                            });
    ...
    function stateContext(el) {
        ...
        if (stateData && stateData.state && stateData.state.name) return stateData.state; else return void 0;
    }
        ...
        return {
            ...
            link: function(scope, element, attrs, uiSrefActive) {
                ...
                angular.forEach(allowedOptions, function(option) {
                    if (option in optionsOverride) options[option] = optionsOverride[option];
                });
                var update = function(newVal) {
                    ...
                    if (nav) {
                        ...
                        if (activeDirective) activeDirective.$$setStateInfo(ref.state, params);
                        if (null === newHref) return nav = !1, !1; else return void attrs.$set(attr, newHref);
                    }
                };
                if (ref.paramExpr) scope.$watch(ref.paramExpr, function(newVal) {
                    if (newVal !== params) update(newVal);
                }, !0), params = angular.copy(scope.$eval(ref.paramExpr));
                if (update(), !isForm) element.bind("click", function(e) {
    ...
    function $StateRefActiveDirective($state, $stateParams, $interpolate) {
        return {
            ...
            controller: [ "$scope", "$element", "$attrs", function($scope, $element, $attrs) {
                function update() {
                    if (isMatch()) $element.addClass(activeClass); else $element.removeClass(activeClass);
                }
                function isMatch() {
                    if ("undefined" != typeof $attrs.uiSrefActiveEq) return state && $state.is(state.name, params); else return state && $state.includes(state.name, params);
                }
    ...
    UrlMatcher.prototype.concat = function(pattern, config) {
        ...
        for (i = 0; nPath > i; i++) {
            ...
            if (paramVal && param.array === !0) paramVal = decodePathArray(paramVal);
        ...
        if (!isDefined(param)) return this.$$paramNames; else return this.params[param] || null;
        ...
        for (i = 0; nTotal > i; i++) {
            ...
            if (isPathParam) {
                ...
                if (squash === !1) {
                    if (null != encoded) if (isArray(encoded)) result += map(encoded, encodeDashes).join("-"); else result += encodeURIComponent(encoded);
                    result += nextSegment;
                } else if (squash === !0) {
                    var capture = result.match(/\/$/) ? /\/?(.*)/ : /(.*)/;
                    result += nextSegment.match(capture)[1];
                } else if (isString(squash)) result += squash + nextSegment;
            } else {
        ...
        function ArrayType(type, mode) {
            ...
            function arrayHandler(callback, allTruthyMode) {
                return function(val) {
                    ...
                    if (allTruthyMode === !0) return 0 === filter(result, falsey).length; else return arrayUnwrap(result);
                };
            }
    ...
    fcsaNumberModule = angular.module("fcsa-number", []), fcsaNumberModule.directive("fcsaNumber", [ "fcsaNumberConfig", function(fcsaNumberConfig) {
        ...
        return defaultOptions = fcsaNumberConfig.defaultOptions, getOptions = function(scope) {
            var option, options, value, _ref;
            if (options = angular.copy(defaultOptions), null != scope.options) {
                _ref = scope.$eval(scope.options);
                for (option in _ref) if (__hasProp.call(_ref, option)) value = _ref[option], options[option] = value; else ;
            }
            return options;
        }, isNumber = function(val) {
            return !isNaN(parseFloat(val)) && isFinite(val);
        }, isNotDigit = function(which) {
            return 44 > which || which > 57 || 47 === which;
        }, controlKeys = [ 0, 8, 13 ], isNotControlKey = function(which) {
            return -1 === controlKeys.indexOf(which);
        }, hasMultipleDecimals = function(val) {
            return null != val && val.toString().split(".").length > 2;
        }, makeMaxDecimals = function(maxDecimals) {
            var regexString, validRegex;
            if (maxDecimals > 0) regexString = "^-?\\d*\\.?\\d{0," + maxDecimals + "}$"; else regexString = "^-?\\d*$";
            return validRegex = new RegExp(regexString), function(val) {
            ...
            var validRegex;
            return validRegex = new RegExp("^-?\\d{0," + maxDigits + "}(\\.\\d*)?$"), function(val) {
                return validRegex.test(val);
            };
        }, makeIsValid = function(options) {
            var validations;
            if (validations = [], null != options.maxDecimals) validations.push(makeMaxDecimals(options.maxDecimals));
            if (null != options.max) validations.push(makeMaxNumber(options.max));
            if (null != options.min) validations.push(makeMinNumber(options.min));
            if (null != options.maxDigits) validations.push(makeMaxDigits(options.maxDigits));
            return function(val) {
                var i, number, _i, _ref;
                if (!isNumber(val)) return !1;
                if (hasMultipleDecimals(val)) return !1;
                for (number = Number(val), i = _i = 0, _ref = validations.length; _ref >= 0 ? _ref > _i : _i > _ref; i = _ref >= 0 ? ++_i : --_i) if (!validations[i](val, number)) return !1;
                return !0;
            };
        }, addCommasToInteger = function(val) {
            var commas, decimals, wholeNumbers;
            return decimals = -1 == val.indexOf(".") ? "" : val.replace(/^-?\d+(?=\.)/, ""), 
            wholeNumbers = val.replace(/(\.\d+)$/, ""), commas = wholeNumbers.replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"), 
            "" + commas + decimals;
        }, {
            restrict: "A",
            require: "ngModel",
            ...
            link: function(scope, elem, attrs, ngModelCtrl) {
                var isValid, options;
                if (options = getOptions(scope), isValid = makeIsValid(options), ngModelCtrl.$parsers.unshift(function(viewVal) {
                    var noCommasVal;
                    if (noCommasVal = viewVal.replace(/,/g, ""), isValid(noCommasVal) || !noCommasVal) return ngModelCtrl.$setValidity("fcsaNumber", !0), 
                    noCommasVal; else return void ngModelCtrl.$setValidity("fcsaNumber", !1);
                }), ngModelCtrl.$formatters.push(function(val) {
                    if (null != options.nullDisplay && (!val || "" === val)) return options.nullDisplay;
                    if (null == val || !isValid(val)) return val;
                    if (ngModelCtrl.$setValidity("fcsaNumber", !0), val = addCommasToInteger(val.toString()), 
                    null != options.prepend) val = "" + options.prepend + val;
                    if (null != options.append) val = "" + val + options.append;
                    return val;
                }), elem.on("blur", function() {
                    var formatter, viewValue, _i, _len, _ref;
                    if (viewValue = ngModelCtrl.$modelValue, null != viewValue && isValid(viewValue)) {
                        for (_ref = ngModelCtrl.$formatters, _i = 0, _len = _ref.length; _len > _i; _i++) formatter = _ref[_i], 
                        viewValue = formatter(viewValue);
                        return ngModelCtrl.$viewValue = viewValue, ngModelCtrl.$render();
                    }
                }), elem.on("focus", function() {
                    var val;
                    if (val = elem.val(), null != options.prepend) val = val.replace(options.prepend, "");
                    if (null != options.append) val = val.replace(options.append, "");
                    return elem.val(val.replace(/,/g, "")), elem[0].select();
                }), options.preventInvalidInput === !0) return elem.on("keypress", function(e) {
                    if (isNotDigit(e.which) && isNotControlKey(e.which)) return e.preventDefault(); else return void 0;
                }); else return void 0;
            }
        };
    } ]), fcsaNumberModule.provider("fcsaNumberConfig", function() {
        var _defaultOptions;
        _defaultOptions = {}, this.setDefaultOptions = function(defaultOptions) {
